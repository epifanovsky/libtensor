...
/** \brief Block tensor interface (complex)
    \tparam N Block tensor order.
    \tparam T Block tensor element type.
...
**/
template<size_t N, typename T>
class btensor_i< N, std::complex<T> > :
    public any_tensor< N, std::complex<T> > {

public:
    btensor_i() : any_tensor< N, std::complex<T> >(*this) {
        eval_btensor_holder< std::complex<T> >::get_instance().inc_counter();
    }

    virtual ~btensor_i() {
        eval_btensor_holder< std::complex<T> >::get_instance().dec_counter();
    }

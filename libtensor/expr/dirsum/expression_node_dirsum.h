#ifndef LIBTENSOR_EXPRESSION_NODE_DIRSUM_H
#define LIBTENSOR_EXPRESSION_NODE_DIRSUM_H
...
/** \brief Expression node: direct sum of two tensors (base class)

    \ingroup libtensor_expr
 **/
template<size_t N, typename T>
class expression_node_dirsum_base : public expression_node<N, T> {
public:
...
public:
     ...
     **/
    virtual ~expression_node_dirsum_base() { }
...
public:
     ...
     **/
    static expression_node_dirsum_base<N, T> &cast(
        expression_node<N, T> &n) {

        return dynamic_cast<expression_node_dirsum_base<N, T>&>(n);
    }

};


/** \brief Expression node: direct sum of two tensors

    \ingroup libtensor_expr
 **/
template<size_t N, size_t M, typename T>
class expression_node_dirsum : public expression_node_dirsum_base<N + M, T> {
private:
...
public:
     ...
     **/
    expression_node_dirsum(expression<N, T> &a, expression<M, T> &b,
        const permutation<N + M> &perm) :
        m_a(a), m_b(b), m_perm(perm), m_s(1) { }

    /** \brief Virtual destructor
     **/
    virtual ~expression_node_dirsum() { }

    /** \brief Clones the node
     **/
    virtual expression_node<N + M, T> *clone() const {
        return new expression_node_dirsum<N, M, T>(*this);
    }
...
public:
    /** \brief Casts an abstract node as expression_node_dirsum
     **/
    static expression_node_dirsum<N, M, T> &cast(
        expression_node<N + M, T> &n) {

        return dynamic_cast<expression_node_dirsum<N, M, T>&>(n);
    }
...
template<size_t N, typename T>
const char *expression_node_dirsum_base<N, T>::k_node_type = "dirsum";


} // namespace libtensor

#endif // LIBTENSOR_EXPRESSION_NODE_DIRSUM_H

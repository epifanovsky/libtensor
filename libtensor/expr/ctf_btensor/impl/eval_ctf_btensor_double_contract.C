...
using eval_btensor_double::meta_if;
using eval_btensor_double::meta_min;


template<size_t NC>
class eval_contract_impl : public eval_ctf_btensor_evaluator_i<NC, double> {
public:
    enum {
    ...
    if(nc.do_contract()) {
        ...
        if(k > na || k > nb) {
            throw eval_exception(__FILE__, __LINE__,
                "libtensor::expr::eval_ctf_btensor_double",
...
void eval_contract_impl<NC>::init_contract(
    ...
    ctf_btensor_from_node<N + K, double> bta(m_tree, e[0]);
    ctf_btensor_from_node<M + K, double> btb(m_tree, e[1]);
    ...
    m_op = new ctf_btod_contract2<N, M, K>(contr,
...
void eval_contract_impl<NC>::init_ewmult(const tensor_transf<NC, double> &trc) {
    ...
    m_op = new ctf_btod_ewmult2<N, M, K>(bta.get_btensor(), perma,

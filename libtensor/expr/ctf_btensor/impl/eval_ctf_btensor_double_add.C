#include <memory>
#include <vector>
#include <libtensor/ctf_block_tensor/ctf_btod_sum.h>
...
template<size_t N>
class eval_add_impl : public eval_ctf_btensor_evaluator_i<N, double> {
private:
    enum {
        Nmax = add<N>::Nmax
    };
...
private:
    std::vector<eval_ctf_btensor_evaluator_i<N, double>*>
        m_sub; //!< Subexpressions

...
template<size_t N>
class eval_div_impl : public eval_ctf_btensor_evaluator_i<N, double> {
private:
    enum {
        Nmax = div<N>::Nmax
    };
...
eval_div_impl<N>::eval_div_impl(const expr_tree &tree,
    ...
    ctf_btensor_from_node<N, double> bta(tree, e[0]);
    ctf_btensor_from_node<N, double> btb(tree, e[1]);

    tensor_transf<N, double> tra(bta.get_transf()), trb(btb.get_transf());
    tra.permute(tr.get_perm());
    trb.permute(tr.get_perm());

    m_op = new ctf_btod_mult<N>(bta.get_btensor(), tra, btb.get_btensor(), trb,

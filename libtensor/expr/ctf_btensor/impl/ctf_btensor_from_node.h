#ifndef LIBTENSOR_EXPR_CTF_BTENSOR_FROM_NODE_H
#define LIBTENSOR_EXPR_CTF_BTENSOR_FROM_NODE_H
...
public:
    ctf_btensor_from_node(const expr_tree &tree, expr_tree::node_id_t head);
    ...
    ctf_btensor<N, T> &get_or_create_btensor(const block_index_space<N> &bis);
...
template<size_t N, typename T>
ctf_btensor_from_node<N, T>::ctf_btensor_from_node(const expr_tree &tree,
...
template<size_t N, typename T>
ctf_btensor<N, T> &ctf_btensor_from_node<N, T>::get_btensor() const {
    ...
    if(n.check_type<node_ident>()) {
        ...
        return ctf_btensor<N, T>::from_any_tensor(ni.get_tensor());

    } else if(n.check_type<node_ctf_btensor_interm_base>()) {

        const node_ctf_btensor_interm<N, T> &ni =
    	    n.template recast_as< node_ctf_btensor_interm<N, T> >();
        ctf_btensor_placeholder<N, T> &ph =
            ctf_btensor_placeholder<N, T>::from_any_tensor(ni.get_tensor());
        if(ph.is_empty()) {
...
template<size_t N, typename T>
ctf_btensor<N, T> &ctf_btensor_from_node<N, T>::get_or_create_btensor(
...
expr_tree::node_id_t transf_from_node(
    ...
    if(n.check_type<node_ident>()) {
        ...
        if(N != nt.get_perm().size()) {
            throw eval_exception(__FILE__, __LINE__,
                "libtensor::expr::eval_ctf_btensor_double", 0,
                "transf_from_node()",
...
#endif // LIBTENSOR_EXPR_CTF_BTENSOR_FROM_NODE_H

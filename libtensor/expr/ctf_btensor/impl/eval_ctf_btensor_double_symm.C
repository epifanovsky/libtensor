...
template<size_t N>
class eval_symm_impl : public eval_ctf_btensor_evaluator_i<N, double> {
private:
    enum {
        Nmax = symm<N>::Nmax
    };
...
private:
    ...
    eval_ctf_btensor_evaluator_i<N, double> *m_sub; //!< Subexpression
...
void eval_symm_impl<N>::init(const tensor_transf<N, double> &tr,
    ...
    if(e.size() != 1) {
        throw eval_exception(__FILE__, __LINE__,
            "libtensor::expr::eval_ctf_btensor_double", "eval_symm_impl<N>",
    ...
    m_op = new ctf_btod_symmetrize2<N>(m_sub->get_bto(), tspr.get_perm(),
...
void eval_symm_impl<N>::init(const tensor_transf<N, double> &tr,
    ...
    m_op = new ctf_btod_symmetrize3<N>(m_sub->get_bto(), tspr1.get_perm(),
...
void eval_symm_impl<N>::init(const tensor_transf<N, double> &tr,
    ...
    throw not_implemented("libtensor::expr::eval_ctf_btensor_double",

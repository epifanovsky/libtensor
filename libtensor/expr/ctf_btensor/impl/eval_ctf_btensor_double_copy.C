...
template<size_t N>
class eval_copy_impl : public eval_ctf_btensor_evaluator_i<N, double> {
private:
    enum {
        Nmax = copy<N>::Nmax
    };
...
eval_copy_impl<N>::eval_copy_impl(const expr_tree &tree,
    ...
    ctf_btensor_from_node<N, double> bta(tree, id);
    m_op = new ctf_btod_copy<N>(bta.get_btensor(), tr.get_perm(),

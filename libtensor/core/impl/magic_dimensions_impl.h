...
template<size_t N>
const magic_dimensions<N> &magic_dimensions<N>::operator=(
    const magic_dimensions<N> &mdims) {

    typedef sequence<N, libdivide::libdivide_u64_t> seq_t;

    seq_t &magic = *reinterpret_cast<seq_t*>(m_magic);
    const seq_t &other_magic = *reinterpret_cast<const seq_t*>(mdims.m_magic);
    magic = other_magic;
    return *this;
}

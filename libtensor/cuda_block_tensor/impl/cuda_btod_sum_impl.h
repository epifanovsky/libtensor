#ifndef LIBTENSOR_CUDA_BTOD_SUM_IMPL_H
#define LIBTENSOR_CUDA_BTOD_SUM_IMPL_H
...
#include "../cuda_btod_sum.h"

namespace libtensor {


template<size_t N>
const char* cuda_btod_sum<N>::k_clazz = "cuda_btod_sum<N>";


template<size_t N>
inline cuda_btod_sum<N>::cuda_btod_sum(additive_gen_bto<N, bti_traits> &op, double c) :
...
template<size_t N>
void cuda_btod_sum<N>::perform(gen_block_stream_i<N, bti_traits> &out) {

    if(m_ops.empty()) return;

    cuda_block_tensor< N, double, cuda_allocator_t > bt(m_bis);
    ...
    cuda_btod_copy<N>(bt).perform(out);
}


template<size_t N>
void cuda_btod_sum<N>::compute_block(
        bool zero,
        const index<N> &i,
        const tensor_transf<N, double> &tr,
        dense_tensor_wr_i<N, double> &blk) {

    if(zero) cuda_tod_set<N>().perform(blk);
...
template<size_t N>
void cuda_btod_sum<N>::perform(gen_block_tensor_i<N, bti_traits> &btb) {

    gen_bto_aux_copy<N, cuda_btod_traits> out(m_sym, btb);
    out.open();
    perform(out);
    out.close();
}
...
template<size_t N>
void cuda_btod_sum<N>::perform(gen_block_tensor_i<N, bti_traits> &btb, double c) {
...
template<size_t N>
void cuda_btod_sum<N>::add_op(additive_gen_bto<N, bti_traits> &op, double c) {
...
template<size_t N>
void cuda_btod_sum<N>::make_schedule() const {
...
#endif // LIBTENSOR_CUDA_BTOD_SUM_IMPL_H

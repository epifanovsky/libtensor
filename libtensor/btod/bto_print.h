...
public:
    ...
    void perform(block_tensor_rd_i<N,T> &bt) throw(exception);
};
...
template<size_t N, typename T, typename Alloc>
void bto_print<N, T, Alloc>::perform(
        block_tensor_rd_i<N, T> &bt) throw(exception)
{
...
using btod_print = bto_print<N, double, Alloc>;

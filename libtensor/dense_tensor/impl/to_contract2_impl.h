...
void to_contract2<N, M, K, T>::perform(bool zero,
    ...
    try {
        ...
        bool zero1 = zero;
        T *pc1 = 0, *pc2 = 0;
        typename allocator<T>::pointer_type vpc;
        vpc = allocator<T>::allocate(dimsc.get_size());
        pc1 = allocator<T>::lock_rw(vpc);
        ...
        allocator<T>::unlock_rw(vpc); pc1 = 0;
        allocator<T>::deallocate(vpc);
...
void to_contract2<N, M, K, T>::perform_internal(aligned_args &ar,
    ...
    typename allocator<T>::pointer_type vpa, vpb;

    pa2 = pa = ca.req_const_dataptr();
    if(!ar.perma.is_identity()) {

        vpa = allocator<T>::allocate(dimsa1.get_size());
        pa1 = allocator<T>::lock_rw(vpa);
    ...
    if(!ar.permb.is_identity()) {
        ...
        vpb = allocator<T>::allocate(dimsb1.get_size());
        pb1 = allocator<T>::lock_rw(vpb);
    ...
    if(pa1) {
        allocator<T>::unlock_rw(vpa); pa1 = 0;
        allocator<T>::deallocate(vpa);
    }
    ca.ret_const_dataptr(pa);

    if(pb1) {
        allocator<T>::unlock_rw(vpb); pb1 = 0;
        allocator<T>::deallocate(vpb);
    }

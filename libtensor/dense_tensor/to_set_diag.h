#ifndef LIBTENSOR_TO_SET_DIAG_H
#define LIBTENSOR_TO_SET_DIAG_H
...
class to_set_diag : public noncopyable {
public:
...
public:
     ...
     **/
    to_set_diag(const sequence<N, size_t> &msk, T v = 0.0) :
        m_msk(msk), m_v(v) { }

    /** \brief Initializes the operation (compatability wrapper)
        \param v Tensor element value (default 0.0).
     **/
    to_set_diag(T v = 0.0) : m_msk(1), m_v(v) { }
...
template<size_t N>
using tod_set_diag = to_set_diag<N, double>; 

} // namespace libtensor

#endif // LIBTENSOR_TO_SET_DIAG_H

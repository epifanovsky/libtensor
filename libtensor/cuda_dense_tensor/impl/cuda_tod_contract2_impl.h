...
template<size_t N, size_t M, size_t K>
void cuda_tod_contract2<N, M, K>::prefetch() {
...
void cuda_tod_contract2<N, M, K>::perform(bool zero,
    ...
    cuda_tod_contract2::start_timer();
...
template<size_t N, size_t M, size_t K>
void cuda_tod_contract2<N, M, K>::align(
...
void cuda_tod_contract2<N, M, K>::perform_internal(aligned_args &ar,
    ...
    if(!ar.perma.is_identity()) {
        ...
        vpa = cuda_allocator<double>::allocate(dimsa1.get_size());
        pa1 = cuda_allocator<double>::lock_rw(vpa);
    ...
    if(!ar.permb.is_identity()) {
        ...
        vpb = cuda_allocator<double>::allocate(dimsb1.get_size());
        pb1 = cuda_allocator<double>::lock_rw(vpb);
...
#endif // LIBTENSOR_CUDA_TOD_CONTRACT2_IMPL_H

#ifndef LIBTENSOR_CUDA_DENSE_TENSOR_H
#define LIBTENSOR_CUDA_DENSE_TENSOR_H
...
/** \brief Simple CUDA %tensor, which stores all its elements in memory

    \param N Tensor order.
    \param T Tensor element type.
    \param Alloc Memory allocator.

    This class is a container for %tensor elements located in GPU memory.
    ...
    The CUDA %tensor class does not perform any mathematical operations, nor
    ...
    Tensor uses a memory allocator to obtain storage.
...
class cuda_dense_tensor :
    public cuda_dense_tensor_i<N, T>,
    public immutable,
    public timings< cuda_dense_tensor<N, T, Alloc> > {
...
public:
    ...
    typedef typename cuda_dense_tensor_i<N, T>::session_handle_type
        handle_t; //!< Session handle type

private:
    dimensions<N> m_dims; //!< Tensor dimensions
    ptr_t m_data; //!< Pointer to data
    cuda_pointer<T> m_dataptr; //!< Pointer to checked out data
    cuda_pointer<const T> m_const_dataptr; //!< Constant pointer to checked out data
...
public:
     ...
     **/
    cuda_dense_tensor(const dimensions<N> &dims);

    /** \brief Creates an empty tensor with the same dimensions
            (data are not copied)
        \param t Another tensor (cuda_dense_tensor_i<N, T>).
     **/
    cuda_dense_tensor(const cuda_dense_tensor_i<N, T> &t);

    /** \brief Creates an empty tensor with the same dimensions (data are not
            copied)
        \param t Another tensor (tensor<N, T, Alloc).
     **/
    cuda_dense_tensor(const cuda_dense_tensor<N, T, Alloc> &t);

    /** \brief Virtual destructor
     **/
    virtual ~cuda_dense_tensor();

    //@}


    //! \name Implementation of libtensor::cuda_dense_tensor_i<N, T>
    //@{
...
protected:
    ...
    virtual cuda_pointer<T> on_req_dataptr(const handle_t &h);
    virtual void on_ret_dataptr(const handle_t &h, cuda_pointer<T> p);
    virtual cuda_pointer<const T> on_req_const_dataptr(const handle_t &h);
    virtual void on_ret_const_dataptr(const handle_t &h, cuda_pointer<const T> p);
...
#endif // LIBTENSOR_CUDA_DENSE_TENSOR_H

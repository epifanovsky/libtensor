...
#include "kern_mul_ijkl_pliq_jpqk.h"


namespace libtensor {


const char *kern_mul_ijkl_pliq_jpqk::k_clazz = "kern_mul_ijkl_pliq_jpqk";


void kern_mul_ijkl_pliq_jpqk::run(const loop_registers<2, 1> &r) {

    if(m_sia == m_nq && m_sla == m_sia * m_ni && m_spa == m_sla * m_nl &&
        m_sqb == m_nk && m_spb == m_sqb * m_nq &&
        m_sjb == m_spb * m_np && m_skc == m_nl &&
        m_sjc == m_skc * m_nk && m_sic == m_sjc * m_nj) {

        linalg::ijkl_pliq_jpqk_x(m_ni, m_nj, m_nk, m_nl, m_np, m_nq,
...
kernel_base<2, 1> *kern_mul_ijkl_pliq_jpqk::match(
    ...
    //  nj  0    sjb  sjc
    //  np  spa  spb  0
    //  ni  sia  0    sic  -->  c_i#j#k#l = a_p#l#i#q b_j#p#q#k
    //  -----------------       [ijkl_pliq_jpqk]
    //
    ...
    return new kern_mul_ijkl_pliq_jpqk(zz);
}

...
kernel_base<2, 1> *kern_mul_ij_ipq_pjq::match(const kern_dmul2_ij_ip_jp &z,
    ...
    if(kern = kern_mul_ijk_jpq_ipkq::match(zz, in, out)) return kern;
    if(kern = kern_mul_ijk_jpq_pikq::match(zz, in, out)) return kern;

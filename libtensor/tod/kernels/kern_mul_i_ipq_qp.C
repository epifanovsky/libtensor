...
kernel_base<2, 1> *kern_mul_i_ipq_qp::match(const kern_dmul2_x_pq_qp &z,
    ...
    if(kern = kern_mul_ij_ipq_jqp::match(zz, in, out)) return kern;
    if(kern = kern_mul_ij_jpq_iqp::match(zz, in, out)) return kern;
    if(kern = kern_mul_ij_jipq_qp::match(zz, in, out)) return kern;

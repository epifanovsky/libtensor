...
const char *kern_mul_ij_piq_pjq::k_clazz = "kern_mul_ij_piq_pjq";


void kern_mul_ij_piq_pjq::run(const loop_registers<2, 1> &r) {
...
kernel_base<2, 1> *kern_mul_ij_piq_pjq::match(const kern_dmul2_ij_ip_jp &z,
    ...
    //  nj  0    sjb  1
    //  ni  sia  0    sic
    //  np  spa  spb  0    -->  c_i#j = a_p#i#q b_p#j#q
    //  -----------------       [ij_piq_pjq]
    //
    ...
    if(kern = kern_mul_ijk_piq_jpkq::match(zz, in, out)) return kern;
    if(kern = kern_mul_ijk_piq_pjkq::match(zz, in, out)) return kern;

    return new kern_mul_ij_piq_pjq(zz);
}

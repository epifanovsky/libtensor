...
const char *kern_mul_ijk_piq_jpqk::k_clazz = "kern_mul_ijk_piq_jpqk";


void kern_mul_ijk_piq_jpqk::run(const loop_registers<2, 1> &r) {
    ...
    for(size_t p = 0; p < m_np; p++) {
        ...
        for(size_t j = 0; j < m_nj; j++) {
            linalg::mul2_ij_ip_pj_x(m_ni, m_nk, m_nq, pa, m_sia,
...
kernel_base<2, 1> *kern_mul_ijk_piq_jpqk::match(const kern_mul_ijk_ip_jpk &z,
    ...
    //  ni  sia  0    sic
    //  nj  0    sjb  sjc
    //  np  spa  spb  0    --> c_i#j#k = a_p#i#q b_j#p#q#k
    //  -----------------      [ijk_piq_jpqk]
    //
    ...
    if(kern = kern_mul_ijkl_pkiq_jpql::match(zz, in, out)) return kern;

    return new kern_mul_ijk_piq_jpqk(zz);
}

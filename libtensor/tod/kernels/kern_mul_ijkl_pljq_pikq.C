...
const char *kern_mul_ijkl_pljq_pikq::k_clazz = "kern_mul_ijkl_pljq_pikq";


void kern_mul_ijkl_pljq_pikq::run(const loop_registers<2, 1> &r) {

    if(m_sja == m_nq && m_skb == m_nq && m_skc == m_nl &&
        m_sla == m_sja * m_nj && m_sib == m_skb * m_nk &&
        m_sjc == m_skc * m_nk && m_spa == m_sla * m_nl &&
        m_spb == m_sib * m_ni && m_sic == m_sjc * m_nj) {

        linalg::ijkl_pikq_pljq_x(m_ni, m_nj, m_nk, m_nl, m_np, m_nq,
...
kernel_base<2, 1> *kern_mul_ijkl_pljq_pikq::match(
    ...
    //  np  spa  spb  0
    //  nj  sja  0    sjc
    //  ni  0    sib  sic  -->  c_i#j#k#l = a_p#l#j#q b_p#i#k#q
    //  -----------------       [ijkl_pljq_pikq]
    ...
    return new kern_mul_ijkl_pljq_pikq(zz);
}

...
#include "kern_mul_ijkl_pkiq_jplq.h"

namespace libtensor {


const char *kern_mul_ijkl_pkiq_jplq::k_clazz = "kern_mul_ijkl_pkiq_jplq";


void kern_mul_ijkl_pkiq_jplq::run(const loop_registers<2, 1> &r) {

    if(m_sia == m_nq && m_slb == m_nq && m_skc == m_nl &&
        m_ska == m_sia * m_ni && m_spb == m_slb * m_nl &&
        m_sjc == m_skc * m_nk && m_spa == m_ska * m_nk &&
        m_sjb == m_spb * m_np && m_sic == m_sjc * m_nj) {

        linalg::ijkl_pkiq_jplq_x(m_ni, m_nj, m_nk, m_nl, m_np, m_nq,
...
kernel_base<2, 1> *kern_mul_ijkl_pkiq_jplq::match(
    const kern_mul_ijk_piq_jpkq &z, list_t &in, list_t &out) {
    ...
    //  np  spa  spb  0
    //  nj  0    sjb  sjc
    //  nk  ska  0    skc  -->  c_i#j#k#l = a_p#k#i#q b_j#p#l#q
    //  -----------------       [ijkl_pkiq_jplq]
    //
    ...
    return new kern_mul_ijkl_pkiq_jplq(zz);
}

...
public:
    ...
    //Copy constructor
    sparse_block_tree(const sparse_block_tree& rhs);

    //Assignment operator
    sparse_block_tree& operator=(const sparse_block_tree& rhs);

    //Destructor
    virtual ~sparse_block_tree();

    //Returns the child/granchild/etc corresponding to the specified sub-key
    const sparse_block_tree& get_sub_tree(const std::vector<key_t>& sub_key) const;

    //Can't use permutation<N> class because permutation degree may need to be determined at runtime
    sparse_block_tree permute(const runtime_permutation& perm) const;
    ...
    sparse_block_tree contract(size_t contract_idx,const std::vector< sparse_bispace<1> >& subspaces) const;

    //Fuses one sparse tree onto this one at position fuse_pos
    //By default, fuses to the branches of the tree
    sparse_block_tree fuse(const sparse_block_tree& rhs,const std::vector<size_t>& lhs_indices,
                                                                                    const std::vector<size_t>& rhs_indices) const;
    //Convenience wrapper for the most common (end to end) case
    sparse_block_tree fuse(const sparse_block_tree& rhs) const;
    ...
    bool operator==(const sparse_block_tree& rhs) const;
    bool operator!=(const sparse_block_tree& rhs) const;
...
private:
    ...
    std::vector< sparse_block_tree* > m_children;
    //For leaf nodes only
    ...
    template<typename container>
    sparse_block_tree(const container& key,size_t key_order,size_t order);

    //Used by permute/fuse/contract to create new instances
    //Does not do the same input validation as primary constructor 
    sparse_block_tree(const std::vector< std::vector<key_t> >& sig_blocks,size_t order);

    static const char *k_clazz; //!< Class name
};

template<typename container>
sparse_block_tree::sparse_block_tree(const container& key,size_t key_order,size_t node_order)
{
    m_order = node_order;
    m_n_entries = 0;
...
template<typename container>
void sparse_block_tree::push_back(const container& key,size_t key_order)
{
    ...
    if((m_keys.size() == 0) || (m_keys.back() != key_entry))
    {
        ...
        if(m_order > 1)
        {
            m_children.push_back(new sparse_block_tree(key,key_order,m_order - 1));
        }
        else
        {

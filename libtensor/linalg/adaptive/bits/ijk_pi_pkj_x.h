#ifndef LIBTENSOR_LINALG_ADAPTIVE_IJK_PI_PKJ_X_H
#define LIBTENSOR_LINALG_ADAPTIVE_IJK_PI_PKJ_X_H

namespace libtensor {


template<typename M, typename L1, typename L2, typename L3>
void linalg_base_level4_adaptive<M, L1, L2, L3>::ijk_pi_pkj_x(
    ...
    if(sjc == nk) {
        L3::ij_pi_pj_x(ni, njk, np, a, spa, b1, njk1, c, sic, d);
    } else {
        double *c1 = M::allocate(ni * njk);
        for(size_t ijk = 0; ijk < ni * njk; ijk++) c1[ijk] = 0.0;
        L3::ij_pi_pj_x(ni, njk, np, a, spa, b1, njk1, c1, njk, 1.0);
...
#endif // LIBTENSOR_LINALG_ADAPTIVE_IJK_PI_PKJ_X_H

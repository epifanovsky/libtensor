#ifndef LIBTENSOR_BTO_SELECT_H
#define LIBTENSOR_BTO_SELECT_H
...
class bto_select : public noncopyable {
public:
    static const char *k_clazz; //!< Class name

    typedef ComparePolicy compare_type;
    typedef gen_bto_select<N, bto_traits<T>, compare_type> bto_select_gen;
    typedef typename bto_select_gen::block_tensor_element_type
            block_tensor_element_type;
    typedef std::list<block_tensor_element_type> list_type;

private:
    bto_select_gen m_gbto; //!< Compare policy object to select entries
...
public:
     ...
     **/
    bto_select(block_tensor_rd_i<N, T> &bt,
     ...
     **/
    bto_select(block_tensor_rd_i<N, T> &bt,
            const symmetry<N, T> &sym, compare_type cmp = compare_type());
...
using btod_select = bto_select<N, double, ComparePolicy>;
...
#include "impl/bto_select_impl.h"

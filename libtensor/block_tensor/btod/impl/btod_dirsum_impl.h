...
class btod_dirsum_task : public libutil::task_i {
private:
    btod_dirsum<N, M> &m_bto;
...
template<size_t N, size_t M, typename T>
class btod_dirsum_task_iterator : public libutil::task_iterator_i {
private:
    btod_dirsum<N, M> &m_bto;
    block_tensor_i<N + M, T> &m_btc;
    bto_stream_i<N + M, btod_traits> &m_out;
...
template<size_t N, size_t M, typename T>
class btod_dirsum_task_observer : public libutil::task_observer_i {
public:
...
void btod_dirsum<N, M>::perform(bto_stream_i<N + M, btod_traits> &out) {
    ...
    try {
        ...
        block_tensor<N + M, double, allocator_type> btc(m_sym_bld.get_bis());
        block_tensor_ctrl<N + M, double> cc(btc);
        cc.req_sync_on();
        sync_on();

        btod_dirsum_task_iterator<N, M, double> ti(*this, btc, out);
        btod_dirsum_task_observer<N, M, double> to;
...
void btod_dirsum<N, M>::perform(block_tensor_i<N + M, double> &btb) {
    ...
    bto_aux_copy<N + M, btod_traits> out(m_sym_bld.get_symmetry(), btb);
    perform(out);
}
...
void btod_dirsum<N, M>::perform(block_tensor_i<N + M, double> &btb,
    ...
    addition_schedule<N + M, btod_traits> asch(m_sym_bld.get_symmetry(),
        cb.req_const_symmetry());
    asch.build(get_schedule(), cb);

    bto_aux_add<N + M, btod_traits> out(m_sym_bld.get_symmetry(), asch, btb, c);
    perform(out);
}
...
void btod_dirsum<N, M>::do_block_scatter_a(
	...
	if(zero) {
		tod_scatter<N, M>(blka, ka, permc).perform(true, blkc);
		if(kc != 1.0) {
			tod_scale<k_orderc>(kc).perform(blkc);
		}
	} else {
		tod_scatter<N, M>(blka, ka * kc, permc).perform(false, blkc);
	}
...
void btod_dirsum<N, M>::do_block_scatter_b(
	...
	if(zero) {
		tod_scatter<M, N>(blkb, kb, permc).perform(true, blkc);
		if(kc != 1.0) {
			tod_scale<k_orderc>(kc).perform(blkc);
		}
	} else {
		tod_scatter<M, N>(blkb, kb * kc, permc).perform(false, blkc);
	}
...
template<size_t N, size_t M, typename T>
btod_dirsum_task<N, M, T>::btod_dirsum_task(btod_dirsum<N, M> &bto,
...
template<size_t N, size_t M, typename T>
void btod_dirsum_task<N, M, T>::perform() {
...
template<size_t N, size_t M, typename T>
btod_dirsum_task_iterator<N, M, T>::btod_dirsum_task_iterator(
    btod_dirsum<N, M> &bto,block_tensor_i<N + M, T> &btc,
    bto_stream_i<N + M, btod_traits> &out) :
...
template<size_t N, size_t M, typename T>
bool btod_dirsum_task_iterator<N, M, T>::has_more() const {
...
template<size_t N, size_t M, typename T>
libutil::task_i *btod_dirsum_task_iterator<N, M, T>::get_next() {

    dimensions<N + M> bidims = m_btc.get_bis().get_block_index_dims();
    index<N + M> idx;
    abs_index<N + M>::get_index(m_sch.get_abs_index(m_i), bidims, idx);
    btod_dirsum_task<N, M, T> *t =
        new btod_dirsum_task<N, M, T>(m_bto, m_btc, idx, m_out);
    ++m_i;
    return t;
}
...
template<size_t N, size_t M, typename T>
void btod_dirsum_task_observer<N, M, T>::notify_finish_task(
    libutil::task_i *t) {

...
template<size_t N, typename Traits, typename Timed>
void gen_bto_trace_in_orbit_task::perform() {

    typedef typename Traits::template rd_block_type<NA>::type rd_block_type;
    ...
    try {
        ...
        for (typename orbit<NA, element_type>::iterator ia = oa.begin();
            ...
            index<NA> ia;
            abs_index<NA>::get_index(oa.get_abs_index(ia), m_bidimsa, ia);
            ia.permute(m_perm);
            ...
            tensor_transf<NA, element_type> tra(oa.get_transf(ia));
            tra.permute(m_perm);

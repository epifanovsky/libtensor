...
class version : public libutil::singleton<version> {
 ...
 private:
  static const unsigned k_major = 3;  //!< Major %version number
 ...
 protected:
  ...
  const std::string& get_status_impl() const { return m_status; }

  const std::string& get_string_impl() const { return m_string; }

  const std::list<std::string>& get_authors_impl() const { return m_authors; }

 public:
  /**    \brief Returns the major %version number
   **/
  static unsigned get_major() { return k_major; }

  /**    \brief Returns the minor %version number
   **/
  static unsigned get_minor() { return k_minor; }

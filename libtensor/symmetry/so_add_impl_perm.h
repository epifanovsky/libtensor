...
private:
	void do_perform(const symmetry_operation_params_t &params) const;
};
...
template<size_t N, typename T>
void symmetry_operation_impl< so_add<N, T>, se_perm<N, T> >::perform(
...
void symmetry_operation_impl< so_add<N, T>, se_perm<N, T> >::do_perform(
	const symmetry_operation_params_t &params) const {

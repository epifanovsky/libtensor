...
void product_table_container::add(
		const std::string &id, const product_table_i &pt) throw(bad_parameter) {

	list_t::iterator it = m_tables.find(id);
	if (it != m_tables.end())
		throw bad_parameter(g_ns, k_clazz, "create(const id_t&, size_t)",
				__FILE__, __LINE__, "Table already exists.");

	it = m_tables.insert(m_tables.begin(), pair_t(id, container()));
...
const product_table_i &product_table_container::req_const_table(
	...
	const char *method = "req_const_table(const id_t&)";
	...
	if (it->second.m_checked_out > 0 && it->second.m_rw)
		throw_exc(k_clazz, method,
				"Table already checked out for writing.");

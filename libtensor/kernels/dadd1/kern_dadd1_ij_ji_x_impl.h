...
template<typename LA>
void kern_dadd1_ij_ji_x<LA>::run(const loop_registers<1, 1> &r) {

    LA::add1_ij_ji_x(0, m_ni, m_nj, r.m_ptra[0], m_sja, m_d, r.m_ptrb[0],
        m_sib);
}


template<typename LA>
kernel_base<1, 1> *kern_dadd1_ij_ji_x<LA>::match(const kern_dadd1_i_i_x<LA> &z,

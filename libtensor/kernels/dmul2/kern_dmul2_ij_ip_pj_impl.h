...
template<typename LA>
void kern_dmul2_ij_ip_pj<LA>::run(const loop_registers<2, 1> &r) {

    LA::mul2_ij_ip_pj_x(0, m_ni, m_nj, m_np, r.m_ptra[0], m_sia,
        r.m_ptra[1], m_spb, r.m_ptrb[0], m_sic, m_d);
}


template<typename LA>
kernel_base<2, 1> *kern_dmul2_ij_ip_pj<LA>::match(

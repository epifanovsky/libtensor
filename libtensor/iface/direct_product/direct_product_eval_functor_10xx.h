...
template<size_t N, size_t M, typename T, typename E1, typename E2,
	size_t NT2, size_t NO2>
void direct_product_eval_functor<N, M, T, E1, E2, 1, 0, NT2, NO2>::create_arg() {

	destroy_arg();
	m_op = new btod_contract2<N, M, 0>(m_contr_bld.get_contr(),
		m_arg_a.get_btensor(), m_eval_b.get_btensor());
	m_arg = new arg<k_orderc, T, oper_tag>(*m_op, m_arg_a.get_coeff());
}


template<size_t N, size_t M, typename T, typename E1, typename E2,
	size_t NT2, size_t NO2>
void direct_product_eval_functor<N, M, T, E1, E2, 1, 0, NT2, NO2>::destroy_arg() {

...
//#include <libtensor/cuda_block_tensor/cuda_btod_add.h>
...
#include <libtensor/cuda_block_tensor/cuda_btod_sum.h>
...
void cuda_btod_sum_test::perform() throw(libtest::test_exception) {
...
void btod_sum_test::test_1() throw(libtest::test_exception) {
    ...
    typedef cuda_block_tensor<2, double, cuda_allocator_t> cuda_block_tensor_t;
    ...
    cuda_block_tensor_t bt1_d(bis), bt2_d(bis), bt3_d(bis);
    ...
    cuda_btod_add<2> add_d(bt1_d);
    add_d.add_op(bt2_d, 2.0);
    add_d.add_op(bt2_d);

    cuda_btod_sum<2> sum_d(add_d);
    sum_d.perform(bt3_d);
...
void cuda_btod_sum_test::test_5() throw(libtest::test_exception) {

    //
    //  Single operand A * B
    //

    static const char *testname = "cuda_btod_sum_test::test_5()";
    ...
    cuda_block_tensor<4, double, cuda_allocator_t> bta_d(bis_iaaa), btb_d(bis_iiaa), btc_d(bis_iaaa);
...
void cuda_btod_sum_test::test_8() throw(libtest::test_exception) {

    //
    //  Two operands with different permutational symmetry
    //

    static const char *testname = "cuda_btod_sum_test::test_8()";
    ...
    block_tensor<2, double, allocator_t> bt1(bis), bt2(bis), bt3(bis),
        bt3_ref(bis);
    cuda_block_tensor<2, double, cuda_allocator_t> bt1_d(bis), bt2_d(bis), bt3_d(bis);
...
//    cp2.perform(bt3_ref, 1.0);
	...
	cuda_btod_copy_h2d<2>(bt1).perform(bt1_d);
	cuda_btod_copy_h2d<2>(bt2).perform(bt2_d);

	//Run operation on GPU

	cuda_btod_copy<2> cp1_d(bt1_d);
	cuda_btod_copy<2> cp2_d(bt2_d);

	cuda_btod_sum<2> sum_d(cp1_d);
	sum_d.add_op(cp2_d);
	sum_d.perform(bt3_d);

	//  Copy back from device to host memory

	cuda_btod_copy_d2h<2>(bt3_d).perform(bt3);
//
...
void cuda_btod_sum_test::test_9a() throw(libtest::test_exception) {

    //
    //  tt_oovv(i|j|a|b) = t2(i|j|a|b) - t1(j|a)*t1(i|b);
    //

    static const char *testname = "cuda_btod_sum_test::test_9a()";
    ...
    cuda_block_tensor<2, double, cuda_allocator_t> bt1_d(bis_ia);
    cuda_block_tensor<4, double, cuda_allocator_t> bt2_d(bis_ijab), bt3_d(bis_ijab);
	...
	cuda_btod_copy_h2d<2>(bt1).perform(bt1_d);
	cuda_btod_copy_h2d<4>(bt2).perform(bt2_d);

	//Run operation on GPU

	cuda_btod_copy<4> op1_d(bt2_d);
	cuda_btod_contract2<2, 2, 0> op2_d(contr1, bt1_d, bt1_d);

	cuda_btod_sum<4> sum_d(op1_d);
	sum_d.add_op(op2_d, -1.0);
	sum_d.perform(bt3_d);

	//  Copy back from device to host memory

	cuda_btod_copy_d2h<4>(bt3_d).perform(bt3);

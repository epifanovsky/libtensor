...
void blas_isomorphism_test::perform() throw(libtest::test_exception) {
    ...
    //Not sure what correct result was intended to be for this test
...
void blas_isomorphism_test::test_matmul_isomorphism_params_3d_3d_A_perm_B_perm() throw(libtest::test_exception)
{
    ...
    vector<sparse_bispace_impl> bispaces(1,spb_i|spb_j|spb_m|spb_l);
    bispaces.push_back(spb_i|spb_k|spb_j);
    bispaces.push_back(spb_l|spb_k|spb_m);

    vector<idx_pair_list> ts_groups(5);
    //i loop
    ts_groups[0].push_back(idx_pair(0,0));
    ts_groups[0].push_back(idx_pair(1,0));
    //j loop
    ...
    ts_groups[1].push_back(idx_pair(1,2));
    //l loop
    ts_groups[2].push_back(idx_pair(0,3));
    ts_groups[2].push_back(idx_pair(2,0));
    //m loop
    ts_groups[3].push_back(idx_pair(0,2));
    ts_groups[3].push_back(idx_pair(2,2));
    //k loop
    ts_groups[4].push_back(idx_pair(1,1));
    ts_groups[4].push_back(idx_pair(2,1));
    ...
    runtime_permutation correct_A_perm(3);
    correct_A_perm.permute(1,2);
    runtime_permutation correct_B_perm(3);
    correct_B_perm.permute(0,2);
    correct_B_perm.permute(1,2);

    if(perm_A !=  correct_A_perm || perm_B != correct_B_perm || A_trans != false || B_trans != true)
    {
        fail_test(test_name,__FILE__,__LINE__,
                "matmul_isomorphism_params returned incorrect value for 3d_3d_A_perm_B_perm case");
    }
}
#endif

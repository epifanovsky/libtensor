...
void sparse_block_tree_iterator_test::test_begin_3d() throw(libtest::test_exception)
{
    ...
    impl::sparse_block_tree<3>& sbt = tf.tree;
    ...
    //Check value
    impl::sparse_block_tree<3>::value_t correct_val(1,std::pair<size_t,size_t>(0,8));
    if((*it) != correct_val)
    {
        fail_test(test_name,__FILE__,__LINE__,
...
void sparse_block_tree_iterator_test::test_end_3d() throw(libtest::test_exception)
{
    ...
    impl::sparse_block_tree<3> sbt(block_tuples_list,subspaces);

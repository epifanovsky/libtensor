...
void sparse_btensor_test::test_contract2_3d_2d() throw(libtest::test_exception)
{
    ...
    sparse_btensor<2> B(tf.spb_B,tf.B_arr,true);
    sparse_btensor<3> C(tf.spb_C,tf.C_correct_arr,true);

    letter i,j,k,l;
    C(i|j|l) = contract(k,A(i|j|k),B(k|l));

    sparse_btensor<3> C_correct(tf.spb_C,tf.C_correct_arr,true);
    if(C != C_correct)
    {
        fail_test(test_name,__FILE__,__LINE__,
                "contract(...) did not produce correct result");
...
void sparse_btensor_test::test_contract2_two_indices_3d_3d_sparse_sparse() throw(libtest::test_exception)
{
    static const char *test_name = "sparse_btensor_test::test_contract2_two_indices_3d_3d_sparse_sparse()";

...
void labeled_btensor_test::test_expr_copy_1() throw(libtest::test_exception) {
    ...
    block_tensor_ctrl<2, double> btctrla(bta), btctrlb(btb),
        btctrlb_ref(btb_ref);
    index<2> i0;
    dense_tensor_wr_i<2, double> &ta = btctrla.req_block(i0);
    dense_tensor_wr_i<2, double> &tb = btctrlb.req_block(i0);
    dense_tensor_wr_i<2, double> &tb_ref = btctrlb_ref.req_block(i0);

    dimensions<2> dims(ta.get_dims());

    {
        dense_tensor_wr_ctrl<2, double> tca(ta), tcb(tb), tcb_ref(tb_ref);
...
void labeled_btensor_test::test_expr_add_1() throw(libtest::test_exception) {
    ...
    dense_tensor_wr_i<2, double> &ta = btctrla.req_block(i0);
    dense_tensor_wr_i<2, double> &tb = btctrlb.req_block(i0);
    dense_tensor_wr_i<2, double> &tc = btctrlc.req_block(i0);
    dense_tensor_wr_i<2, double> &tc_ref = btctrlc_ref.req_block(i0);

    dimensions<2> dims(ta.get_dims());

    {
        dense_tensor_wr_ctrl<2, double> tca(ta), tcb(tb), tcc(tc),
            tcc_ref(tc_ref);
        double *dta = tca.req_dataptr();

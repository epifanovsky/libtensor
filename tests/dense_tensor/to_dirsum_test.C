...
void to_dirsum_test_x<T>::test_ij_i_j_1(size_t ni, size_t nj, T d)
    ...
    tnss << "to_dirsum_test_x<T>::test_ij_i_j_1(" << ni << ", " << nj << ", "
        << d << ")";
    ...
    if(d == 0.0) {
        to_dirsum<1, 1, T>(ta, 1.0, tb, 1.0).perform(true, tc);
    } else {
        scalar_transf<T> s1(1.), sd(d);
        tensor_transf<2, T> trc(permutation<2>(), sd);
        to_dirsum<1, 1, T>(ta, s1, tb, s1, trc).perform(false, tc);
    }
...
void to_dirsum_test_x<T>::test_ij_i_j_2(size_t ni, size_t nj, T d)
    ...
    std::stringstream tnss;
    tnss << "to_dirsum_test_x<T>::test_ij_i_j_2(" << ni << ", " << nj << ", "
        << d << ")";
    std::string tns = tnss.str();
    ...
    if(d == 0.0) {
        ...
        tensor_transf<2, T> trc(permutation<2>(), sd);
        to_dirsum<1, 1, T>(ta, s1, tb, s2, trc).perform(false, tc);
    }
...
void to_dirsum_test_x<T>::test_ikj_ij_k_1(size_t ni, size_t nj, size_t nk,
    ...
    std::stringstream tnss;
    tnss << "to_dirsum_test_x<T>::test_ikj_ij_k(" << ni << ", " << nj << ", "
        << nk << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    if(d == 0.0) {
        to_dirsum<2, 1, T>(ta, 1.0, tb, 1.0, permc).perform(true, tc);
    } else {
        scalar_transf<T> s1(1.), sd(d);
        tensor_transf<3, T> trc(permc, sd);
        to_dirsum<2, 1, T>(ta, s1, tb, s1, trc).perform(false, tc);
    }
...
void to_dirsum_test_x<T>::test_ikjl_ij_kl_1(size_t ni, size_t nj, size_t nk,
    ...
    std::stringstream tnss;
    tnss << "to_dirsum_test_x<T>::test_ikjl_ij_kl(" << ni << ", " << nj << ", "
        << nk << ", " << nl << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    if(d == 0.0) {
        to_dirsum<2, 2, T>(ta, 1.0, tb, 1.0, permc).perform(true, tc);
    } else {
        scalar_transf<T> s1(1.), sd(d);
        tensor_transf<4, T> trc(permc, sd);
        to_dirsum<2, 2, T>(ta, s1, tb, s1, trc).perform(false, tc);
    }
...
void to_dirsum_test_x<T>::test_iklj_ij_kl_1(size_t ni, size_t nj, size_t nk,
    ...
    std::stringstream tnss;
    tnss << "to_dirsum_test_x<T>::test_iklj_ij_kl(" << ni << ", " << nj << ", "
        << nk << ", " << nl << ", " << d << ")";
    std::string tns = tnss.str();

...
#include "to_contract2_test.h"
#include "to_contract2_test_x.h"
...
//template<typename T>
//const T to_contract2_test_x<T>::k_thresh = 5e-14;

template<>
const double to_contract2_test_x<double>::k_thresh = 7e-14;

template<>
const float to_contract2_test_x<float>::k_thresh = 1e-5;

void to_contract2_test::perform() throw(libtest::test_exception) {
    std::cout << "Testing to_contract2<double>  ";
    to_contract2_test_x<double> td;
    td.perform();

    std::cout << "Testing to_contract2<float>  ";
    to_contract2_test_x<float> tf;
    tf.perform();
}


template<typename T>
void to_contract2_test_x<T>::perform() throw(libtest::test_exception) {
...
template<typename T>
void to_contract2_test_x<T>::test_0_p_p(size_t np, T d)
    throw(libtest::test_exception) {

    // c = \sum_p a_p b_p

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_0_p_p(" << np << ", " << d << ")";
    ...
    dense_tensor<1, T, allocator<T> > ta(dima);
    dense_tensor<1, T, allocator<T> > tb(dimb);
    dense_tensor<0, T, allocator<T> > tc(dimc);
    dense_tensor<0, T, allocator<T> > tc_ref(dimc);
    ...
    dense_tensor_ctrl<0, T> tcc(tc);
    dense_tensor_ctrl<0, T> tcc_ref(tc_ref);
    ...
    contr.contract(0, 0);
    if(d == 0.0) to_contract2<0, 0, 1>(contr, ta, tb).perform(true, tc);
    else to_contract2<0, 0, 1>(contr, ta, tb, d).perform(false, tc);

    // Compare against the reference

    compare_ref_x<0>::compare(tns.c_str(), tc, tc_ref, cij_max * k_thresh);
...
template<typename T>
void to_contract2_test_x<T>::test_i_p_pi(size_t ni, size_t np, T d)
    throw(libtest::test_exception) {

    // c_i = \sum_p a_p b_{pi}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_i_p_pi(" << ni << ", " << np << ", "
        << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<1, T, allocator<T> > ta(dima);
    dense_tensor<2, T, allocator<T> > tb(dimb);
    dense_tensor<1, T, allocator<T> > tc(dimc);
    dense_tensor<1, T, allocator<T> > tc_ref(dimc);
    ...
    contr.contract(0, 0);
    if(d == 0.0) to_contract2<0, 1, 1, T>(contr, ta, tb).perform(true, tc);
    else to_contract2<0, 1, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_i_p_ip(size_t ni, size_t np, T d)
    throw(libtest::test_exception) {

    // c_i = \sum_p a_p b_{ip}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_i_p_ip(" << ni << ", " << np << ", "
        << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_i_pi_p(size_t ni, size_t np, T d)
    throw(libtest::test_exception) {

    // c_i = \sum_p a_{pi} b_p

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_i_pi_p(" << ni << ", " << np << ", "
        << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<2, T, allocator<T> > ta(dima);
    dense_tensor<1, T, allocator<T> > tb(dimb);
    dense_tensor<1, T, allocator<T> > tc(dimc);
    dense_tensor<1, T, allocator<T> > tc_ref(dimc);
    ...
    contr.contract(0, 0);
    if(d == 0.0) to_contract2<1, 0, 1, T>(contr, ta, tb).perform(true, tc);
    else to_contract2<1, 0, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_i_ip_p(size_t ni, size_t np, T d)
    throw(libtest::test_exception) {

    // c_i = \sum_p a_{ip} b_p

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_i_ip_p(" << ni << ", " << np << ", "
        << d << ")";
    std::string tns = tnss.str();
    ...
    contraction2<1, 0, 1> contr;
    contr.contract(1, 0);
    if(d == 0.0) to_contract2<1, 0, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<1, 0, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_i_j(size_t ni, size_t nj, T d)
    throw(libtest::test_exception) {

    // c_{ij} = c_{ij} + d a_{i} b_{j}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_i_j(" << ni << ", " << nj
        << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<1, T, allocator<T> > ta(dima);
    dense_tensor<1, T, allocator<T> > tb(dimb);
    dense_tensor<2, T, allocator<T> > tc(dimc);
    dense_tensor<2, T, allocator<T> > tc_ref(dimc);
    ...
    if(d == 0.0) to_contract2<1, 1, 0, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<1, 1, 0, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_j_i(size_t ni, size_t nj, T d)
    throw(libtest::test_exception) {

    // c_{ij} = c_{ij} + d a_{j} b_{i}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_j_i(" << ni << ", " << nj
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pi_pj(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {

    // c_{ij} = \sum_p a_{pi} b_{pj}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pi_pj(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    contr.contract(0, 0);
    if(d == 0.0) to_contract2<1, 1, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<1, 1, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pi_jp(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pi_jp(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_ip_pj(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_ip_pj(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_ip_jp(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_ip_jp(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pj_pi(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pj_pi(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pj_ip(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pj_ip(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_jp_ip(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_jp_ip(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_jp_pi(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_jp_pi(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_p_pji(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_p_pji(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<1, T, allocator<T> > ta(dima);
    dense_tensor<3, T, allocator<T> > tb(dimb);
    dense_tensor<2, T, allocator<T> > tc(dimc);
    dense_tensor<2, T, allocator<T> > tc_ref(dimc);
    ...
    contr.contract(0, 0);
    if(d == 0.0) to_contract2<0, 2, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<0, 2, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pji_p(
    size_t ni, size_t nj, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pji_p(" << ni << ", " << nj
        << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<3, T, allocator<T> > ta(dima);
    dense_tensor<1, T, allocator<T> > tb(dimb);
    dense_tensor<2, T, allocator<T> > tc(dimc);
    dense_tensor<2, T, allocator<T> > tc_ref(dimc);
    ...
    contr.contract(0, 0);
    if(d == 0.0) to_contract2<2, 0, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<2, 0, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pi_pj_qi_jq(
    size_t ni, size_t nj, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {

    //  c_{ij} = \sum_p a^1_{pi} b^1_{pj} + \sum_q a^2_{qi} b^2_{jq}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pi_pj_qi_jq(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<2, T, allocator<T> > ta1(dima1), ta2(dima2);
    dense_tensor<2, T, allocator<T> > tb1(dimb1), tb2(dimb2);
    dense_tensor<2, T, allocator<T> > tc(dimc);
    dense_tensor<2, T, allocator<T> > tc_ref(dimc);
    T d1, d2;
    ...
    {
    dense_tensor_ctrl<2, T> tca1(ta1), tca2(ta2);
    dense_tensor_ctrl<2, T> tcb1(tb1), tcb2(tb2);
    dense_tensor_ctrl<2, T> tcc(tc);
    dense_tensor_ctrl<2, T> tcc_ref(tc_ref);
    T *dta1 = tca1.req_dataptr();
    T *dta2 = tca2.req_dataptr();
    T *dtb1 = tcb1.req_dataptr();
    T *dtb2 = tcb2.req_dataptr();
    T *dtc1 = tcc.req_dataptr();
    T *dtc2 = tcc_ref.req_dataptr();
    ...
    to_contract2<1, 1, 1, T> op(contr1, ta1, tb1, d1 * k);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pi_pj_qi_qj(
    size_t ni, size_t nj, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pi_pj_qi_qj(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_ip_pkj(
    size_t ni, size_t nj, size_t nk, size_t np, T d)
    throw(libtest::test_exception) {

    // c_{ijk} = \sum_p a_{ip} b_{pkj}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_ip_pkj(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<2, T, allocator<T> > ta(dima);
    dense_tensor<3, T, allocator<T> > tb(dimb);
    dense_tensor<3, T, allocator<T> > tc(dimc);
    dense_tensor<3, T, allocator<T> > tc_ref(dimc);
    ...
    contr.contract(1, 0);
    if(d == 0.0) to_contract2<1, 2, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<1, 2, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pi_pkj(
    size_t ni, size_t nj, size_t nk, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pi_pkj(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pik_pj(
    size_t ni, size_t nj, size_t nk, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pik_pj(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    contr.contract(0, 0);
    if(d == 0.0) to_contract2<2, 1, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<2, 1, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pj_ipk(
    size_t ni, size_t nj, size_t nk, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pj_ipk(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pj_pik(
    size_t ni, size_t nj, size_t nk, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pj_pik(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pkj_ip(
    size_t ni, size_t nj, size_t nk, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pkj_ip(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pkj_pi(
    size_t ni, size_t nj, size_t nk, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pkj_pi(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pqi_pjq(
    size_t ni, size_t nj, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pqi_pjq(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    contr.contract(0, 0);
    contr.contract(1, 2);
    if(d == 0.0) to_contract2<1, 1, 2, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<1, 1, 2, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_ipq_jqp(
    size_t ni, size_t nj, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_ipq_jqp(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_jpq_iqp(
    size_t ni, size_t nj, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_jpq_iqp(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_jipq_qp(
    size_t ni, size_t nj, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_jipq_qp(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<4, T, allocator<T> > ta(dima);
    dense_tensor<2, T, allocator<T> > tb(dimb);
    dense_tensor<2, T, allocator<T> > tc(dimc);
    dense_tensor<2, T, allocator<T> > tc_ref(dimc);
    ...
    contr.contract(2, 1);
    contr.contract(3, 0);
    if(d == 0.0) to_contract2<2, 0, 2, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<2, 0, 2, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pq_ijpq(size_t ni, size_t nj, size_t np,
    size_t nq) throw(libtest::test_exception) {

    // c_{ij} = \sum_{pq} a_{pq} b_{ijpq}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pq_ijpq(" << ni << ", " << nj
        << ", " << np << ", " << nq << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<2, T, allocator<T> > ta(dima);
    dense_tensor<4, T, allocator<T> > tb(dimb);
    dense_tensor<2, T, allocator<T> > tc(dimc);
    dense_tensor<2, T, allocator<T> > tc_ref(dimc);
    ...
    to_contract2<0, 2, 2, T>(contr, ta, tb, 1.0).perform(true, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pq_ijpq_a(size_t ni, size_t nj, size_t np,
    size_t nq, T d) throw(libtest::test_exception) {

    // c_{ij} = c_{ij} + d \sum_{pq} a_{pq} b_{ijpq}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pq_ijpq_a(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    to_contract2<0, 2, 2, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_kjpq_iqp(size_t ni, size_t nj, size_t nk,
    size_t np, size_t nq, T d) throw(libtest::test_exception) {

    // c_{ijk} = \sum_{pq} a_{kjpq} b_{iqp}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_kjpq_iqp(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    contr.contract(2, 2);
    contr.contract(3, 1);
    if(d == 0.0) to_contract2<2, 1, 2, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<2, 1, 2, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pkiq_pjq(size_t ni, size_t nj, size_t nk,
    size_t np, size_t nq, T d) throw(libtest::test_exception) {

    // c_{ijk} = c_{ijk} + d \sum_{pq} a_{pkiq} b_{pjq}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pkiq_pjq(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pqj_iqpk(size_t ni, size_t nj, size_t nk,
    size_t np, size_t nq, T d) throw(libtest::test_exception) {

    // c_{ijk} = \sum_{pq} a_{pqj} b_{iqpk}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pqj_iqpk(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    contr.contract(0, 2);
    contr.contract(1, 1);
    if(d == 0.0) to_contract2<1, 2, 2, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<1, 2, 2, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijk_pqji_qpk(size_t ni, size_t nj, size_t nk,
    size_t np, size_t nq, T d) throw(libtest::test_exception) {

    // c_{ijk} = \sum_{pq} a_{pqji} b_{qpk}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijk_pqji_qpk(" << ni << ", " << nj
        << ", " << nk << ", " << np << ", " << nq << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_ikp_jpl(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, T d) throw(libtest::test_exception) {

    // c_{ijkl} = c_{ijkl} + d \sum_{p} a_{ikp} b_{jpl}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ikp_jpl(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<3, T, allocator<T> > ta(dima);
    dense_tensor<3, T, allocator<T> > tb(dimb);
    dense_tensor<4, T, allocator<T> > tc(dimc);
    dense_tensor<4, T, allocator<T> > tc_ref(dimc);
    ...
    contr.contract(2, 1);
    if(d == 0.0) to_contract2<2, 2, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<2, 2, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_ipk_jpl(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, T d) throw(libtest::test_exception) {

    // c_{ijkl} = c_{ijkl} + d \sum_{p} a_{ikp} b_{jpl}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ipk_jpl(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_ipl_jpk(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, T d) throw(libtest::test_exception) {

    // c_{ijkl} = c_{ijkl} + d \sum_{p} a_{ipl} b_{jpk}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ipl_jpk(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_jkp_ipl(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, T d) throw(libtest::test_exception) {

    // c_{ijkl} = c_{ijkl} + d \sum_{p} a_{jkp} b_{ipl}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_jkp_ipl(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_jpl_ipk(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, T d) throw(libtest::test_exception) {

    // c_{ijkl} = c_{ijkl} + d \sum_{p} a_{jpl} b_{ipk}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_jpl_ipk(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_jpl_ipk_jiq_kql_jlr_ikr(size_t ni, size_t nj,
    size_t nk, size_t nl, size_t np, size_t nq, size_t nr, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_jpl_ipk_jiq_kql_jlr_ikr("
        << ni << ", " << nj << ", " << nk << ", " << nl << ", " << np << ", "
        << nq << ", " << nr << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<3, T, allocator<T> > ta1(dima1), ta2(dima2), ta3(dima3);
    dense_tensor<3, T, allocator<T> > tb1(dimb1), tb2(dimb2), tb3(dimb3);
    dense_tensor<4, T, allocator<T> > tc(dimc);
    dense_tensor<4, T, allocator<T> > tc_ref(dimc);
    T d1, d2, d3;
    ...
    {
    dense_tensor_ctrl<3, T> tca1(ta1), tca2(ta2), tca3(ta3);
    dense_tensor_ctrl<3, T> tcb1(tb1), tcb2(tb2), tcb3(tb3);
    dense_tensor_ctrl<4, T> tcc(tc);
    dense_tensor_ctrl<4, T> tcc_ref(tc_ref);
    T *dta1 = tca1.req_dataptr();
    T *dta2 = tca2.req_dataptr();
    T *dta3 = tca3.req_dataptr();
    T *dtb1 = tcb1.req_dataptr();
    T *dtb2 = tcb2.req_dataptr();
    T *dtb3 = tcb3.req_dataptr();
    T *dtc1 = tcc.req_dataptr();
    T *dtc2 = tcc_ref.req_dataptr();
    ...
    to_contract2<2, 2, 1, T> op(contr1, ta1, tb1, d1 * k);
    op.add_args(contr2, ta2, tb2, d2 * k);
...
template<typename T>
void to_contract2_test_x<T>::test_ijklm_ikp_jpml(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t nm, size_t np, T d)
    throw(libtest::test_exception) {

    // c_{ijklm} = c_{ijklm} + d \sum_{p} a_{ikp} b_{jpml}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijklm_ikp_jpml(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << nm << ", " << np
        << ", "  << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<3, T, allocator<T> > ta(dima);
    dense_tensor<4, T, allocator<T> > tb(dimb);
    dense_tensor<5, T, allocator<T> > tc(dimc);
    dense_tensor<5, T, allocator<T> > tc_ref(dimc);
    ...
    dense_tensor_ctrl<3, T> tca(ta);
    dense_tensor_ctrl<4, T> tcb(tb);
    dense_tensor_ctrl<5, T> tcc(tc);
    dense_tensor_ctrl<5, T> tcc_ref(tc_ref);
    T *dta = tca.req_dataptr();
    T *dtb = tcb.req_dataptr();
    T *dtc1 = tcc.req_dataptr();
    T *dtc2 = tcc_ref.req_dataptr();
    ...
    contr.contract(2, 1);
    if(d == 0.0) to_contract2<2, 3, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<2, 3, 1, T>(contr, ta, tb, d).perform(false, tc);

    // Compare against the reference

    compare_ref_x<5, T>::compare(tns.c_str(), tc, tc_ref, cij_max * k_thresh);
...
template<typename T>
void to_contract2_test_x<T>::test_ijklm_ipkm_jpl(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t nm, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijklm_ipkm_jpl(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << nm << ", " << np
        << ", "  << d << ")";
    std::string tns = tnss.str();
    ...
    contr.contract(1, 1);
    if(d == 0.0) to_contract2<3, 2, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<3, 2, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijklm_jlp_ipkm(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t nm, size_t np, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijklm_jlp_ipkm(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << nm << ", " << np
        << ", "  << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijklmn_kjmp_ipln(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t nm, size_t nn, size_t np, T d)
    throw(libtest::test_exception) {

    // c_{ijklmn} = c_{ijklmn} + d \sum_{p} a_{kjmp} b_{ipln}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijklmn_kjmp_ipln(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << nm << ", " << nn
        << ", "  << np << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    dense_tensor<4, T, allocator<T> > ta(dima);
    dense_tensor<4, T, allocator<T> > tb(dimb);
    dense_tensor<6, T, allocator<T> > tc(dimc);
    dense_tensor<6, T, allocator<T> > tc_ref(dimc);
    ...
    dense_tensor_ctrl<4, T> tca(ta);
    dense_tensor_ctrl<4, T> tcb(tb);
    dense_tensor_ctrl<6, T> tcc(tc);
    dense_tensor_ctrl<6, T> tcc_ref(tc_ref);
    T *dta = tca.req_dataptr();
    T *dtb = tcb.req_dataptr();
    T *dtc1 = tcc.req_dataptr();
    T *dtc2 = tcc_ref.req_dataptr();
    ...
    contr.contract(3, 1);
    if(d == 0.0) to_contract2<3, 3, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<3, 3, 1, T>(contr, ta, tb, d).perform(false, tc);

    // Compare against the reference

    compare_ref_x<6, T>::compare(tns.c_str(), tc, tc_ref, cij_max * k_thresh);
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_iplq_kpjq(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {

    // c_{ijkl} = \sum_{pq} a_{iplq} b_{kpjq}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_iplq_kpjq(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    contr.contract(1, 1);
    contr.contract(3, 3);
    if(d == 0.0) to_contract2<2, 2, 2, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<2, 2, 2, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_iplq_pkjq(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_iplq_pkjq(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_iplq_pkqj(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_iplq_pkqj(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_ipql_kpqj(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ipql_kpqj(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_ipql_pkqj(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ipql_pkqj(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_pilq_kpjq(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_pilq_kpjq(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_pilq_pkjq(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_pilq_pkjq(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_piql_kpqj(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_piql_kpqj(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_piql_pkqj(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_piql_pkqj(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_pqkj_iqpl(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_pqkj_iqpl(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_pqkj_qipl(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, size_t nq, T d)
    throw(libtest::test_exception) {
    ...
    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_pqkj_qipl(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << nq
        << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_ipqr_jpqr(size_t ni, size_t nj, size_t np,
    size_t nq, size_t nr) throw(libtest::test_exception) {

    // c_{ij} = \sum_{pqr} a_{ipqr} b_{jpqr}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_ipqr_jpqr(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << nr << ")";
    std::string tns = tnss.str();
    ...
    to_contract2<1, 1, 3, T>(contr, ta, tb, 1.0).perform(true, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_ipqr_jpqr_a(size_t ni, size_t nj, size_t np,
    size_t nq, size_t nr, T d) throw(libtest::test_exception) {

    // c_{ij} = c_{ij} + d \sum_{pqr} a_{ipqr} b_{jpqr}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_ipqr_jpqr_a(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << nr << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    to_contract2<1, 1, 3, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_ipqr_pjrq(
    size_t ni, size_t nj, size_t np, size_t nq, size_t nr, T d)
    throw(libtest::test_exception) {

    // c_{ij} = \sum_{pq} a_{ipqr} b_{pjrq}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_ipqr_pjrq(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << nr << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    contr.contract(2, 3);
    contr.contract(3, 2);
    if(d == 0.0) to_contract2<1, 1, 3, T>(contr, ta, tb, 1.0).perform(true, tc);
    else to_contract2<1, 1, 3, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ij_jpqr_iprq(size_t ni, size_t nj, size_t np,
    size_t nq, size_t nr, T d) throw(libtest::test_exception) {

    // c_{ij} = c_{ij} + d \sum_{pqr} a_{jpqr} b_{iprq}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_jpqr_iprq(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << nr << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    //~ to_contract2<1, 1, 3, T> op(contr, ta, tb);
    to_contract2<1, 1, 3, T> op(contr, tb, ta, (d != 0 ? d : 1.0));
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pqir_pqjr(size_t ni, size_t nj,
    size_t np, size_t nq, size_t nr) throw(libtest::test_exception) {

    // c_{ij} = \sum_{pqr} a_{pqir} b_{pqjr}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pqir_pqjr(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << nr << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ij_pqir_pqjr_a(size_t ni, size_t nj, size_t np,
    size_t nq, size_t nr, T d) throw(libtest::test_exception) {

    // c_{ij} = c_{ij} + d \sum_{pqr} a_{pqir} b_{pqjr}

    std::stringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ij_pqir_pqjr_a(" << ni << ", " << nj
        << ", " << np << ", " << nq << ", " << nr << ", " << d << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_pi_jklp(size_t ni, size_t nj,
    ...
    tnss << "to_contract2_test_x<T>::test_ijkl_pi_jklp(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ")";
    ...
    to_contract2<1, 3, 1, T>(contr, ta, tb, 1.0).perform(true, tc);

    //
    // Compare against the reference
    //

    compare_ref_x<4, T>::compare(tnss.str().c_str(), tc, tc_ref,
        cijkl_max*k_thresh);
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_pi_jklp_a(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, T d) throw(libtest::test_exception) {
    ...
    std::ostringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_pi_jklp_a(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << d << ")";
    ...
    to_contract2<1, 3, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_jikl_pi_jpkl(size_t ni, size_t nj,
    size_t nk, size_t nl, size_t np) throw(libtest::test_exception) {

    // c_{jikl} = \sum_p a_{pi} b_{jpkl}

    std::ostringstream tnss;
    tnss << "to_contract2_test_x<T>::test_jikl_pi_jpkl(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ")";
    std::string tns = tnss.str();
...
template<typename T>
void to_contract2_test_x<T>::test_jikl_pi_jpkl_a(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, T d) throw(libtest::test_exception) {

    // c_{jikl} = c_{jikl} + d \sum_p a_{pi} b_{jpkl}

    std::ostringstream tnss;
    tnss << "to_contract2_test_x<T>::test_jikl_pi_jpkl_a(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
...
void to_contract2_test_x<T>::test_ijkl_ijp_klp(size_t ni, size_t nj,
    size_t nk, size_t nl, size_t np) throw(libtest::test_exception) {

    // c_{ijkl} = \sum_{p} a_{ijp} b_{klp}

    std::ostringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ijp_klp(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ")";
    std::string tns = tnss.str();
    ...
    to_contract2<2, 2, 1, T>(contr, ta, tb, 1.0).perform(true, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_ijp_klp_a(size_t ni, size_t nj, size_t nk,
    size_t nl, size_t np, T d) throw(libtest::test_exception) {

    // c_{ijkl} = c_{ijkl} + d \sum_{p} a_{ijp} b_{klp}

    std::ostringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ijp_klp_a(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ", " << np << ", " << d << ")";
    std::string tns = tnss.str();
    ...
    to_contract2<2, 2, 1, T>(contr, ta, tb, d).perform(false, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_ij_kl(size_t ni, size_t nj,
    ...
    std::ostringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ij_kl(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ")";
    ...
    to_contract2<2, 2, 0, T>(contr, ta, tb, 1.0).perform(true, tc);
...
template<typename T>
void to_contract2_test_x<T>::test_ijkl_ij_lk(size_t ni, size_t nj,
    size_t nk, size_t nl) throw(libtest::test_exception) {
    ...
    std::ostringstream tnss;
    tnss << "to_contract2_test_x<T>::test_ijkl_ij_lk(" << ni << ", " << nj
        << ", " << nk << ", " << nl << ")";
...
template class to_contract2_test_x<double>;
template class to_contract2_test_x<float>;

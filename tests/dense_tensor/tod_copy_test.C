...
int test_plain(const dimensions<N> &dims) {
    ...
    try {
        ...
        to_copy<N, double>(ta).perform(true, tb);
...
int test_plain_additive(const dimensions<N> &dims, double d) {
    ...
    try {
        ...
        to_copy<N, double>(ta, d).perform(false, tb);
...
int test_scaled(const dimensions<N> &dims, double c) {
    ...
    try {
        ...
        to_copy<N, double>(ta, c).perform(true, tb);
...
int test_scaled_additive(const dimensions<N> &dims, double c, double d) {
    ...
    try {
        ...
        to_copy<N, double>(ta, c * d).perform(false, tb);
...
int test_perm(const dimensions<N> &dims, const permutation<N> &perm) {
    ...
    try {
        ...
        dense_tensor<N, double, allocator> ta(dimsa), tb(dimsb),
            tb_ref(dimsb);
        ...
        to_copy<N, double>(ta, perm).perform(true, tb);
...
int test_perm_additive(const dimensions<N> &dims, const permutation<N> &perm,
    ...
    try {
        ...
        to_copy<N, double>(ta, perm, d).perform(false, tb);
...
int test_perm_scaled_additive(const dimensions<N> &dims,
    ...
    try {
        ...
        tensor_transf<N, double> tr(perm, scalar_transf<double>(c * d));
        to_copy<N, double> cp(ta, tr);
        cp.perform(false, tb);
...
int test_exc() {
    ...
    bool ok = false;
    try {
        to_copy<4, double>(t1).perform(true, t2);
    } catch(exception &e) {
        ok = true;
    }

...
void to_import_raw_stream_test::perform() throw (libtest::test_exception) {
    std::cout << "Testing to_import_raw_stream_test_x<double>   ";
    to_import_raw_stream_test_x<double> t_double;
    t_double.perform();
    std::cout << "Testing to_import_raw_stream_test_x<float>   ";
    to_import_raw_stream_test_x<float> t_float;
    t_float.perform();
}

template<typename T>
void to_import_raw_stream_test_x<T>::perform() throw (libtest::test_exception) {

    index<2> i2_s, i2_e, i2_m1, i2_m2;
    i2_e[0] = 9; i2_e[1] = 19;
    i2_m1[0] = 4; i2_m1[1] = 9;
    i2_m2[0] = 7; i2_m2[1] = 15;
    index_range<2> ir2_se(i2_s, i2_e), ir2_sm(i2_s, i2_m2), ir2_me(i2_m1, i2_e),
        ir2_mm(i2_m1, i2_m2);
    ...
    index_range<4> ir4_se(i4_s, i4_e), ir4_sm(i4_s, i4_m2), ir4_me(i4_m1, i4_e),
        ir4_mm(i4_m1, i4_m2);
...
template<typename T>
template<size_t N>
void to_import_raw_stream_test_x<T>::test_1(const dimensions<N> &dims,
    const index_range<N> &ir) throw (libtest::test_exception) {

    std::ostringstream tnss;
    tnss << "to_import_raw_stream_test_x<T>::test_1(" << dims << ", "
...

    {
        ...
        std::stringstream ss;
        ss.write((const char*)p1_ref, sz1 * sizeof(T));
        tc1.ret_const_dataptr(p1_ref);

        // Invoke the operation

        to_import_raw_stream<N, T>(ss, dims, ir).perform(t2);
    }

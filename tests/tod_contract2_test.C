...
*/
	...
	test_ijkl_pkiq_jplq(3, 5, 2, 7, 13, 11);
	test_ijkl_pkiq_jplq(1, 1, 1, 1, 1, 1, 0.0);
	test_ijkl_pkiq_jplq(1, 1, 1, 1, 1, 1, -0.5);
	test_ijkl_pkiq_jplq(2, 1, 1, 1, 1, 1, 2.0);
	test_ijkl_pkiq_jplq(1, 2, 1, 1, 1, 1, -1.0);
	test_ijkl_pkiq_jplq(1, 1, 2, 1, 1, 1, 3.7);
	test_ijkl_pkiq_jplq(1, 1, 1, 2, 1, 1, 1.0);
	test_ijkl_pkiq_jplq(1, 1, 1, 1, 2, 1, -1.2);
	test_ijkl_pkiq_jplq(1, 1, 1, 1, 1, 2, 0.7);
	test_ijkl_pkiq_jplq(2, 3, 2, 3, 2, 3, 12.3);
	test_ijkl_pkiq_jplq(3, 5, 1, 7, 13, 11, -1.25);
	test_ijkl_pkiq_jplq(3, 5, 2, 7, 13, 11, -1.25);
	...
	test_ijkl_pkiq_pjlq(3, 5, 2, 7, 13, 11);
	test_ijkl_pkiq_pjlq(1, 1, 1, 1, 1, 1, 0.0);
	test_ijkl_pkiq_pjlq(1, 1, 1, 1, 1, 1, -0.5);
	test_ijkl_pkiq_pjlq(2, 1, 1, 1, 1, 1, 2.0);
	test_ijkl_pkiq_pjlq(1, 2, 1, 1, 1, 1, -1.0);
	test_ijkl_pkiq_pjlq(1, 1, 2, 1, 1, 1, 3.7);
	test_ijkl_pkiq_pjlq(1, 1, 1, 2, 1, 1, 1.0);
	test_ijkl_pkiq_pjlq(1, 1, 1, 1, 2, 1, -1.2);
	test_ijkl_pkiq_pjlq(1, 1, 1, 1, 1, 2, 0.7);
	test_ijkl_pkiq_pjlq(2, 3, 2, 3, 2, 3, 12.3);
	test_ijkl_pkiq_pjlq(3, 5, 1, 7, 13, 11, -1.25);
	test_ijkl_pkiq_pjlq(3, 5, 2, 7, 13, 11, -1.25);
/*
...
void tod_contract2_test::test_ijkl_pkiq_jplq(size_t ni, size_t nj, size_t nk,
	size_t nl, size_t np, size_t nq, double d)
	throw(libtest::test_exception) {

	//	c_{ijkl} = \sum_{pq} a_{pkiq} b_{jplq}

	std::stringstream tnss;
	tnss << "tod_contract2_test::test_ijkl_pkiq_jplq(" << ni << ", " << nj
		<< ", " << nk << ", " << nl << ", " << np << ", " << nq
		<< ", " << d << ")";
	std::string tns = tnss.str();
	...
	index<4> ib1, ib2;
	ib2[0] = nj - 1; ib2[1] = np - 1; ib2[2] = nl - 1; ib2[3] = nq - 1;
	index<4> ic1, ic2;
	ic2[0] = ni - 1; ic2[1] = nj - 1; ic2[2] = nk - 1; ic2[3] = nl - 1;
	dimensions<4> dima(index_range<4>(ia1, ia2));
	dimensions<4> dimb(index_range<4>(ib1, ib2));
	...
	contraction2<2, 2, 2> contr(permutation<4>().permute(0, 1).
		permute(1, 2));
	contr.contract(0, 1);
	contr.contract(3, 3);
	if(d == 0.0) tod_contract2<2, 2, 2>(contr, ta, tb).perform(tc);
...
void tod_contract2_test::test_ijkl_pkiq_pjlq(size_t ni, size_t nj, size_t nk,
	size_t nl, size_t np, size_t nq, double d)
	throw(libtest::test_exception) {

	//	c_{ijkl} = \sum_{pq} a_{pkiq} b_{pjlq}

	std::stringstream tnss;
	tnss << "tod_contract2_test::test_ijkl_pkiq_pjlq(" << ni << ", " << nj
		<< ", " << nk << ", " << nl << ", " << np << ", " << nq
		<< ", " << d << ")";
	std::string tns = tnss.str();
	...
	ia2[0] = np - 1; ia2[1] = nk - 1; ia2[2] = ni - 1; ia2[3] = nq - 1;
	index<4> ib1, ib2;
	ib2[0] = np - 1; ib2[1] = nj - 1; ib2[2] = nl - 1; ib2[3] = nq - 1;
	index<4> ic1, ic2;
	ic2[0] = ni - 1; ic2[1] = nj - 1; ic2[2] = nk - 1; ic2[3] = nl - 1;
	dimensions<4> dima(index_range<4>(ia1, ia2));
	dimensions<4> dimb(index_range<4>(ib1, ib2));

#include <libtensor/expr/letter_expr.h>
...
void letter_expr_test::test_contains() throw(libtest::test_exception) {
    ...
    static const char *testname = "letter_expr_test::test_contains()";

    letter i, j, k, l;

    if(!letter_expr<1>(i).contains(i)) {
        fail_test(testname, __FILE__, __LINE__,
            "Failed index location: (i).contains(i)");
    }
    if(!(i|j|k).contains(i)) {
        fail_test(testname, __FILE__, __LINE__,
...
void letter_expr_test::test_permutation() throw(libtest::test_exception) {
    ...
    static const char *testname = "letter_expr_test::test_permutation()";
    ...
    if(!p.equals(p1)) {
        fail_test(testname, __FILE__, __LINE__,
            "Failed permutation test abcd<-abcd");
    }
    ...
    if(!p.equals(p2)) {
        fail_test(testname, __FILE__,  __LINE__,
            "Failed permutation test abcd<-abdc");
    }
    ...
    if(!p.equals(p3)) {
        fail_test(testname, __FILE__, __LINE__,
            "Failed permutation test abcd<-badc");
    }
    ...
    if(!p.equals(p4)) {
        fail_test(testname, __FILE__, __LINE__,
            "Failed permutation test abcd<-acdb");
    }
    ...
    if(!p.equals(p5)) {
        fail_test(testname, __FILE__, __LINE__,
            "Failed permutation test abcd<-bcda");
    }
}

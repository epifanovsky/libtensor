...
void contract_test::test_tt_7() throw(libtest::test_exception) {
    ...
    //  iabc = kcad ikbd
    //  caib->iabc
    contraction2<2, 2, 2> contr(permutation<4>().permute(0, 2).permute(2, 3));
    contr.contract(0, 1);
    contr.contract(3, 3);
...
void contract_test::test_tt_8() throw(libtest::test_exception) {
...

    {
        ...
        c1.req_symmetry().insert(
            se_perm<4, double>(permutation<4>().permute(0, 1), false));
        c2.req_symmetry().insert(
            se_perm<4, double>(permutation<4>().permute(0, 1), false));
    }
...
void contract_test::test_ee_2() throw(libtest::test_exception) {
    ...
    letter a, b, c, d, i;
    t3(a|b|c|d) = contract(i, t1a(a|c|d|i) + t1b(a|i|c|d), t2a(i|b) + t2b(b|i));

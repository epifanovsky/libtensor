...
void diag_tensor_test::test_2() throw(libtest::test_exception) {
    ...
    try {
        ...
        {
            diag_tensor_wr_ctrl<4, double> ctrl(dt);
            ssn1 = ctrl.req_add_subspace(dts1);
            size_t sz1 = dt.get_space().get_subspace_size(ssn1);
            double *p1 = ctrl.req_dataptr(ssn1);
            for(size_t i = 0; i < sz1; i++) p1[i] = 0.1;
            ctrl.ret_dataptr(ssn1, p1); p1 = 0;
        }
        {
            diag_tensor_wr_ctrl<4, double> ctrl(dt);
            ssn2 = ctrl.req_add_subspace(dts2);
            size_t sz2 = dt.get_space().get_subspace_size(ssn2);
            double *p2 = ctrl.req_dataptr(ssn2);
            for(size_t i = 0; i < sz2; i++) p2[i] = -0.1;
            ctrl.ret_dataptr(ssn2, p2); p2 = 0;
        }
        ...
        }
        {
            ...
            double *p1 = ctrl.req_dataptr(ssn1);
            double *p2 = ctrl.req_dataptr(ssn2);
            for(size_t i = 0; i < sz1; i++) p1[i] = 0.2;
            for(size_t i = 0; i < sz2; i++) p2[i] = -0.2;
            ctrl.ret_dataptr(ssn1, p1); p1 = 0;
            ctrl.ret_dataptr(ssn2, p2); p2 = 0;
        }
        {
            ...
            const double *p1 = ctrl.req_const_dataptr(ssn1);
            const double *p2 = ctrl.req_const_dataptr(ssn2);
            for(size_t i = 0; i < sz1; i++) if(p1[i] != 0.2) {
                fail_test(testname, __FILE__, __LINE__, "Bad data in p1.");
            }
            for(size_t i = 0; i < sz2; i++) if(p2[i] != -0.2) {
                fail_test(testname, __FILE__, __LINE__, "Bad data in p2.");
            }
            ctrl.ret_const_dataptr(ssn1, p1); p1 = 0;
            ctrl.ret_const_dataptr(ssn2, p2); p2 = 0;
...
void diag_tensor_test::test_3() throw(libtest::test_exception) {
    ...
    try {
        ...
        {
            diag_tensor_wr_ctrl<4, double> ctrl(dt);
            ctrl.req_remove_subspace(ssn1);
        }
        if(dt.get_space().get_nsubspaces() != 1) {
            fail_test(testname, __FILE__, __LINE__,
                "dt.get_space().get_nsubspaces() != 1");
        }
        {
            diag_tensor_rd_ctrl<4, double> ctrl(dt);
            const double *p2 = ctrl.req_const_dataptr(ssn2);
        ...
        {
            diag_tensor_wr_ctrl<4, double> ctrl(dt);
            ctrl.req_remove_subspace(ssn2);
        }
        if(dt.get_space().get_nsubspaces() != 0) {
            fail_test(testname, __FILE__, __LINE__,
                "dt.get_space().get_nsubspaces() != 0");
        }
...
void diag_tensor_test::test_4() throw(libtest::test_exception) {

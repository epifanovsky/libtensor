...
void linalg_mul2_ij_ip_pj_x_test::perform() throw(libtest::test_exception) {

    test_mul2_ij_ip_pj_x(1, 1, 1, 1, 1, 1);
    test_mul2_ij_ip_pj_x(1, 2, 3, 3, 2, 2);
    test_mul2_ij_ip_pj_x(2, 1, 3, 3, 1, 1);
    test_mul2_ij_ip_pj_x(16, 16, 1, 1, 16, 16);
    test_mul2_ij_ip_pj_x(3, 17, 2, 2, 17, 17);
    test_mul2_ij_ip_pj_x(2, 2, 2, 2, 3, 4);
    test_mul2_ij_ip_pj_x(2, 2, 2, 4, 3, 2);
}


void linalg_mul2_ij_ip_pj_x_test::test_mul2_ij_ip_pj_x(size_t ni, size_t nj,
    size_t np, size_t sia, size_t sic, size_t spb) {

    std::ostringstream ss;
    ss << "linalg_mul2_ij_ip_pj_x_test::test_mul2_ij_ip_pj_x("
        << ni << ", " << nj << ", " << np << ", " << sia << ", "
        << sic << ", " << spb << ")";
    ...
    d = 0.0;
    linalg::mul2_ij_ip_pj_x(0, ni, nj, np, &a[0], sia, &b[0], spb, &c[0], sic,
        d);
    linalg_generic::mul2_ij_ip_pj_x(0, ni, nj, np, &a[0], sia, &b[0], spb,
        &c_ref[0], sic, d);

...
/*
    if(d == 0.0) { // scale diagonal of CTF output by 2
        dense_tensor<2, double, allocator_t> ts(dimsc);
        tod_set<2>(1.0).perform(ts);
        tod_set_diag<2>(2.0).perform(ts);
        tod_mult1<2>(ts).perform(true, tc);
    }
 */

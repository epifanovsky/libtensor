...
void ctf_tod_ewmult2_test::perform() throw(libtest::test_exception) {
...
void ctf_tod_ewmult2_test::test_1a() {

    static const char testname[] = "ctf_tod_ewmult2_test::test_1a()";
    ...
    tensor_transf<1, double> tra(permutation<1>(), scalar_transf<double>(-1.0));
    ...
    tod_ewmult2<0, 1, 1>(ta, tra, tb, trb, trc).perform(true, tc_ref);

    ctf_tod_distribute<1>(ta).perform(dta);
    ctf_tod_distribute<2>(tb).perform(dtb);
    ctf_tod_distribute<2>(tc).perform(dtc);

    ctf_tod_ewmult2<0, 1, 1>(dta, tra, dtb, trb, trc).perform(true, dtc);
...
void ctf_tod_ewmult2_test::test_1b() {

    static const char testname[] = "ctf_tod_ewmult2_test::test_1b()";
    ...
    tensor_transf<2, double> trb(permutation<2>(), scalar_transf<double>(0.5));
    tensor_transf<2, double> trc(permutation<2>(), scalar_transf<double>(1.0));
    tod_ewmult2<0, 1, 1>(ta, tra, tb, trb, trc).perform(false, tc_ref);

    ctf_tod_distribute<1>(ta).perform(dta);
    ctf_tod_distribute<2>(tb).perform(dtb);
    ctf_tod_distribute<2>(tc).perform(dtc);

    ctf_tod_ewmult2<0, 1, 1>(dta, tra, dtb, trb, trc).perform(false, dtc);
    ctf_tod_collect<2>(dtc).perform(tc);

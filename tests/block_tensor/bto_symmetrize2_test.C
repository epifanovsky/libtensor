...
void bto_symmetrize2_test::perform() throw(libtest::test_exception) {
    std::cout << "Testing bto_symmetrize2_test_x<double>   ";
    bto_symmetrize2_test_x<double> t_double;
    t_double.perform();
    std::cout << "Testing bto_symmetrize2_test_x<float>   ";
    bto_symmetrize2_test_x<float> t_float;
    t_float.perform();
}

template<>
const double bto_symmetrize2_test_x<double>::k_thresh = k_thresh;

template<>
const float bto_symmetrize2_test_x<float>::k_thresh = 5e-7;


template<typename T>
void bto_symmetrize2_test_x<T>::perform() throw(libtest::test_exception) {
 ...
 **/
template<typename T>
void bto_symmetrize2_test_x<T>::test_1() throw(libtest::test_exception) {

    static const char *testname = "bto_symmetrize2_test_x<T>::test_1()";
    ...
    bto_copy<2, T> op_copy(bta);
    bto_symmetrize2<2, T>(op_copy, 0, 1, true).perform(btb);

    to_btconv<2, T>(btb).perform(tb);

    //  Compare against the reference: symmetry and data

    symmetry<2, T> symb(bis), symb_ref(bis);
    {
        block_tensor_ctrl<2, T> ctrlb(btb);
        so_copy<2, T>(ctrlb.req_const_symmetry()).perform(symb);
    }
    scalar_transf<T> tr0, tr1(-1.);
    symb_ref.insert(se_perm<2, T>(
        permutation<2>().permute(0, 1), tr0));

    compare_ref_x<2, T>::compare(testname, symb, symb_ref);
 ...
 **/
template<typename T>
void bto_symmetrize2_test_x<T>::test_2() throw(libtest::test_exception) {

    static const char *testname = "bto_symmetrize2_test_x<T>::test_2()";
    ...
    bto_copy<2, T> op_copy(bta);
    bto_symmetrize2<2, T>(op_copy, 0, 1, false).perform(btb);
 ...
 **/
template<typename T>
void bto_symmetrize2_test_x<T>::test_3() throw(libtest::test_exception) {

    static const char *testname = "bto_symmetrize2_test_x<T>::test_3()";
    ...
    block_tensor<4, T, allocator_t> bta(bis), btb(bis), btb_ref(bis);
    ...
    dense_tensor<4, T, allocator_t> ta(dims), tb(dims), tb_ref(dims);
    ...
    bto_symmetrize2<4, T>(op_copy, 1, 3, false).perform(btb);

    to_btconv<4, T>(btb).perform(tb);

    //  Compare against the reference: symmetry and data

    symmetry<4, T> symb(bis), symb_ref(bis);
    {
        block_tensor_ctrl<4, T> ctrlb(btb);
        so_copy<4, T>(ctrlb.req_const_symmetry()).perform(symb);
    }
    scalar_transf<T> tr0, tr1(-1.);
    symb_ref.insert(se_perm<4, T>(
        permutation<4>().permute(0, 2), tr1));
    symb_ref.insert(se_perm<4, T>(
        permutation<4>().permute(1, 3), tr1));

    compare_ref_x<4, T>::compare(testname, symb, symb_ref);
 ...
 **/
template<typename T>
void bto_symmetrize2_test_x<T>::test_4() throw(libtest::test_exception) {

    static const char *testname = "bto_symmetrize2_test_x<T>::test_4()";
    ...
    bto_copy<4, T> op_copy(bta);
    bto_symmetrize2<4, T>(op_copy, 0, 2, true).perform(btb);
 ...
 **/
template<typename T>
void bto_symmetrize2_test_x<T>::test_5(bool symm) throw(libtest::test_exception) {

    static const char *testname = "bto_symmetrize2_test_x<T>::test_5(bool)";
    ...
    bto_copy<4, T> op_copy(bta);
    bto_symmetrize2<4, T>(op_copy, permutation<4>().permute(0, 2).
 ...
 **/
template<typename T>
void bto_symmetrize2_test_x<T>::test_6a(bool symm, bool label,
        bool part, bool doadd) throw(libtest::test_exception) {

    std::ostringstream tnss;
    tnss << "bto_symmetrize2_test_x<T>::test_6a(" << symm << ", "
            << label << ", " << part << ", " << doadd << ")";
    ...
    symmetry<2, T> sym_ref(bis);

    // setup symmetry
    {
    block_tensor_ctrl<2, T> ca(bta), cb(btb);

    scalar_transf<T> tr(symm ? 1.0 : -1.0);
    se_perm<2, T> se10(p, tr);
    cb.req_symmetry().insert(se10);
    sym_ref.insert(se10);

    if (label) {
        se_label<2, T> sl(bis.get_block_index_dims(), tns);
    ...
    bto_copy<2, T> op_copy(bta);
    if (doadd) bto_symmetrize2<2, T>(op_copy, p, symm).perform(btb, 0.25);
    else bto_symmetrize2<2, T>(op_copy, p, symm).perform(btb);
    ...
    compare_ref_x<2, T>::compare(tns.c_str(), ctrlb.req_const_symmetry(), sym_ref);

    compare_ref_x<2, T>::compare(tns.c_str(), tb, tb_ref, k_thresh);
 ...
 **/
template<typename T>
void bto_symmetrize2_test_x<T>::test_6b(bool symm, bool label,
        bool part) throw(libtest::test_exception) {

    std::ostringstream tnss;
    tnss << "bto_symmetrize2_test_x<T>::test_6b(" << symm << ", "
            << label << ", " << part << ", " << ")";
    std::string tns = tnss.str();
    ...
    block_tensor<4, T, allocator_t> bta(bis), btb(bis);
    symmetry<4, T> sym_ref(bis);
    ...
    scalar_transf<T> tr(symm ? 1.0 : -1.0);
    se_perm<4, T> se1(p1, tr);
    se_perm<4, T> se2(p2, tr);
    sym_ref.insert(se1);
    sym_ref.insert(se2);

    if (label) {
        se_label<4, T> sl(bis.get_block_index_dims(), tns);
    ...
    bto_copy<4, T> op_copy(bta);
    bto_symmetrize2<4, T> sym1(op_copy, p1, symm);
    bto_symmetrize2<4, T> sym2(sym1, p2, symm);
    sym2.perform(btb);
    ...
    compare_ref_x<4, T>::compare(tns.c_str(), sym2.get_symmetry(), sym_ref);
    compare_ref_x<4, T>::compare(tns.c_str(), ctrlb.req_const_symmetry(), sym_ref);

    compare_ref_x<4, T>::compare(tns.c_str(), tb, tb_ref, k_thresh);
...
template<typename T>
void bto_symmetrize2_test_x<T>::test_7() {

    const char *testname = "bto_symmetrize2_test_x<T>::test_7()";
    ...
    block_tensor<2, T, allocator_t> bt1(bis_ia);
    block_tensor<4, T, allocator_t> bt2(bis_ijab);

    {
        block_tensor_ctrl<2, T> ctrl(bt1);

        index<2> i00, i01, i10, i11;
        i10[0] = 1; i01[1] = 1;
        i11[0] = 1; i11[1] = 1;

        se_part<2, T> se(bis_ia, m11, 2);
        se.add_map(i00, i11);
    ...
    bto_contract2<2, 2, 0, T> op_contr(contr, bt1, bt1);

    bto_symmetrize2<4, T>(op_contr, 0, 1, false).perform(bt2);

    dense_tensor<2, T, allocator_t> t1(dims_ia);
    dense_tensor<4, T, allocator_t> ti1(dims_ijab), t2(dims_ijab),
        t2_ref(dims_ijab);

    to_btconv<2, T>(bt1).perform(t1);
    to_btconv<4, T>(bt2).perform(t2);

    to_contract2<2, 2, 0, T>(contr, t1, t1).perform(true, ti1);
    to_copy<4, T>(ti1).perform(true, t2_ref);
    to_copy<4, T>(ti1, permutation<4>().permute(0, 1), -1.0).
        perform(false, t2_ref);

    compare_ref_x<4, T>::compare(testname, t2, t2_ref, k_thresh);

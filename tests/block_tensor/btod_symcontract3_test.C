...
#include <libtensor/block_tensor/btod_symcontract3.h>
...
void btod_symcontract3_test::test_contr_1() {
    ...
    try {
        ...
        btod_symcontract3<1, 0, 1, 1, 1>(contr1, contr2, bta, btb,
            permutation<2>().permute(0, 1), true, btc).perform(btd);

        //  Convert block tensors to regular tensors

        dense_tensor<2, double, allocator_t> ta(dims), tb(dims), tab(dims),
            tabs(dims), tc(dims), td(dims), td_ref(dims);
        ...
        tod_copy<2>(tab, permutation<2>(), 1.0).perform(true, tabs);
        tod_copy<2>(tab, permutation<2>().permute(0, 1), 1.0).
            perform(false, tabs);
        tod_contract2<1, 1, 1>(contr2, tabs, tc).perform(true, td_ref);
...
void btod_symcontract3_test::test_contr_2() {
    ...
    try {
        ...
        btod_symcontract3<1, 1, 1, 1, 1>(contr1, contr2, bta, btb,
            permutation<3>().permute(0, 1), true, btc).perform(btd);

        //  Convert block tensors to regular tensors

        dense_tensor<2, double, allocator_t> ta(dims2), tc(dims2);
        dense_tensor<3, double, allocator_t> tb(dims3), tab(dims3), tabs(dims3),
            td(dims3), td_ref(dims3);
        tod_btconv<2>(bta).perform(ta);
        ...
        tod_copy<3>(tab, permutation<3>(), 1.0).perform(true, tabs);
        tod_copy<3>(tab, permutation<3>().permute(0, 1), 1.0).
            perform(false, tabs);
        tod_contract2<2, 1, 1>(contr2, tabs, tc).perform(true, td_ref);
...
void btod_symcontract3_test::test_contr_3() {
    ...
    try {
        ...
        btod_symcontract3<2, 0, 2, 1, 2>(contr1, contr2, bta, bta,
            permutation<4>().permute(0, 1).permute(2, 3), true, btb).
            perform(btc);

        //  Convert block tensors to regular tensors

        dense_tensor<3, double, allocator_t> ta(dimsa);
        dense_tensor<4, double, allocator_t> tb(dimsb), ti(dimsi), tis(dimsi),
            tc(dimsc), tc_ref(dimsc);
        ...
        tod_copy<4>(ti, permutation<4>(), 1.0).perform(true, tis);
        tod_copy<4>(ti, permutation<4>().permute(0, 1).permute(2, 3), 1.0).
            perform(false, tis);
        tod_contract2<2, 2, 2>(contr2, tis, tb).perform(true, tc_ref);
...
void btod_symcontract3_test::test_contr_4() {
    ...
    try {
        ...
        btod_symcontract3<2, 0, 2, 1, 2>(contr1, contr2, bta1, bta2,
            permutation<4>().permute(2, 3), false, btb).perform(btc);
        ...
        tod_copy<4>(ti, permutation<4>(), 1.0).perform(true, tis);
        tod_copy<4>(ti, permutation<4>().permute(2, 3), -1.0).
            perform(false, tis);
        tod_contract2<2, 2, 2>(contr2, tis, tb).perform(true, tc_ref);
...
void btod_symcontract3_test::test_contr_5() {
...
void btod_symcontract3_test::test_contr_6() {
    ...
    try {
        ...
        btod_symcontract3<2, 0, 2, 1, 2>(contr1, contr2, bta, bta,
            permutation<4>().permute(0, 1), false, btb).perform(btc);
        ...
        tod_contract2<2, 2, 1>(contr1, ta, ta).perform(true, ti);
        tod_copy<4>(ti, permutation<4>(), 1.0).perform(true, tis);
        tod_copy<4>(ti, permutation<4>().permute(0, 1), -1.0).
            perform(false, tis);
        tod_contract2<2, 2, 2>(contr2, tis, tb).perform(true, tc_ref);
...
void btod_symcontract3_test::test_contr_7() {

    //
    //  c_{ijkl} = a_{qkr} a_{plr} b_{ijpq}
    //  [k,l,p,q] = 9, [ij] = 5, [r] = 11
    //
    ...
    try {
        ...
        // a_{qkr} a_{plr} -> I_{klpq}
        // qkpl -> klpq
        contraction2<2, 2, 1> contr1(permutation<4>().permute(0, 1).
            permute(1, 3));
        contr1.contract(2, 2);
        // I_{klpq} b_{ijpq} -> c_{ijkl}
        // klij -> ijkl
        ...
        tod_contract2<2, 2, 1>(contr1, ta, ta).perform(true, ti);
        tod_contract2<2, 2, 2>(contr2, ti, tb).perform(true, ttc);
        tod_copy<4>(ttc).perform(true, tc_ref);
        tod_copy<4>(ttc, permutation<4>().permute(2, 3), -1.0).
            perform(false, tc_ref);

        //  Compare against reference

        compare_ref<4>::compare(testname, tc, tc_ref, 5e-13);

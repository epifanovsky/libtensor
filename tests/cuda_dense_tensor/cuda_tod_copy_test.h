#ifndef LIBTENSOR_CUDA_TOD_COPY_TEST_H
#define LIBTENSOR_CUDA_TOD_COPY_TEST_H
...
/** \brief Tests the libtensor::cuda_tod_copy class

    \ingroup libtensor_cuda_dense_tensor_tests
**/
class cuda_tod_copy_test : public libtest::unit_test {
public:
...
private:
     ...
     **/
    template<size_t N>
    void test_perm_scaled(const dimensions<N> &dims, const permutation<N> &perm,
        double c) throw(libtest::test_exception);

    /** \brief Tests permuted and scaled copying of a tensor (additive version)
     **/
    template<size_t N>
    ...
    /** \brief Tests if an exception is throws when the tensors have different
            dimensions
     **/
...
#endif // LIBTENSOR_CUDA_TOD_COPY_TEST_H

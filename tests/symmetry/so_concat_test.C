...
/**	\test Invokes a concatenation of S2 in 2-space and C1 in 1-space to form a
		3-space. Expects C2 in 3-space.
 **/
void so_concat_test::test_1a() throw(libtest::test_exception) {

	static const char *testname = "so_concat_test::test_1a()";
...
/**	\test Invokes a concatenation of C1 in 1-space and S2 in 2-space to form a
		3-space. Expects C2 in 3-space.
 **/
void so_concat_test::test_1b() throw(libtest::test_exception) {

	static const char *testname = "so_concat_test::test_1b()";
	...
	mask<2> m2; m2[0] = true; m2[1] = true;
	bis2.split(m2, 2); bis2.split(m2, 3);
	mask<3> m3a, m3b; m3a[0] = true; m3b[1] = true; m3b[2] = true;
	bis3.split(m3a, 5); bis3.split(m3b, 2); bis3.split(m3b, 3);
	...
	so_concat<1, 2, double>(sym1, sym2).perform(sym3);
...
/**	\test Invokes a concatenation of C3 in 3-space and C1 in 1-space to form a
		4-space with permutation. Expects C3 in 4-space.
 **/
void so_concat_test::test_5a() throw(libtest::test_exception) {

	static const char *testname = "so_concat_test::test_5a()";
	...
	dimensions<3> dims3(index_range<3>(i3a, i3b));
	index<4> i4a, i4b;
	i4b[0] = 5; i4b[1] = 5; i4b[2] = 5; i4b[3] = 10;
	dimensions<4> dims4(index_range<4>(i4a, i4b));
	...
	bis1.split(m1, 5);
	mask<3> m3; m3[0] = true; m3[1] = true; m3[2] = true;
	bis3.split(m3, 2); bis3.split(m3, 3);
	mask<4> m4a, m4b;
	m4a[0] = true; m4a[1] = true; m4a[2] = true; m4b[3] = true;
	bis4.split(m4a, 2); bis4.split(m4a, 3); bis4.split(m4b, 5);

	permutation<4> perm;
	perm.permute(0, 2).permute(1, 3).permute(2, 3);
	bis4.permute(perm);

	symmetry<3, double> sym1(bis3);
	symmetry<1, double> sym2(bis1);
	symmetry<4, double> sym3(bis4);

	symmetry<4, double> sym3_ref(bis4);

	sym1.insert(se_perm<3, double>(
			permutation<3>().permute(0, 1).permute(1, 2), true));
	sym3_ref.insert(se_perm<4, double>(
			permutation<4>().permute(0, 3).permute(2, 3), true));

	so_concat<3, 1, double>(sym1, sym2, perm).perform(sym3);
...
/**	\test Invokes a concatenation of C1 in 1-space and C3 in 3-space to form a
		4-space with permutation. Expects C3 in 4-space.
 **/
void so_concat_test::test_5b() throw(libtest::test_exception) {

	static const char *testname = "so_concat_test::test_5b()";
	...
	dimensions<3> dims3(index_range<3>(i3a, i3b));
	index<4> i4a, i4b;
	i4b[0] = 10; i4b[1] = 5; i4b[2] = 5; i4b[3] = 5;
	dimensions<4> dims4(index_range<4>(i4a, i4b));
	...
	bis3.split(m3, 2); bis3.split(m3, 3);
	mask<4> m4a, m4b;
	m4a[0] = true; m4b[1] = true; m4b[2] = true; m4b[3] = true;
	bis4.split(m4a, 5); bis4.split(m4b, 2); bis4.split(m4b, 3);
	...
	so_concat<1, 3, double>(sym1, sym2, perm).perform(sym3);

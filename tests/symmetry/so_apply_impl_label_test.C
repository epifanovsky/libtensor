...
#include <libtensor/symmetry/label/so_apply_impl_label.h>
...
void so_apply_impl_label_test::test_1(
	...
	tnss << "so_apply_impl_label_test::test_1(" << table_id << ", "
			<< keep_zero << ", " << is_asym << ", " << sign << ")";

	typedef se_label<2, double> se2_t;
	typedef so_apply<2, double> so_t;
	typedef symmetry_operation_impl<so_t, se2_t> so_impl_t;

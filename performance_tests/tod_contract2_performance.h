#ifndef LIBTENSOR_TOD_CONTRACT2_PERFORMANCE_H
#define LIBTENSOR_TOD_CONTRACT2_PERFORMANCE_H
...
**/
template<size_t Repeats, size_t N, size_t M, size_t K, typename DimData>
class tod_contract2_p1 : public performance_test<Repeats> {
protected:
    virtual void do_calculate();
};
...
**/
template<size_t Repeats, size_t N, size_t M, size_t K, typename DimData>
class tod_contract2_p2 : public performance_test<Repeats> {
protected:
    virtual void do_calculate();
};
...
**/
template<size_t Repeats, size_t N, size_t M, size_t K, typename DimData>
class tod_contract2_p4 : public performance_test<Repeats> {
protected:
    virtual void do_calculate();
};
...
const char* tod_contract2_ref<R,N,M,K,DimData>::k_clazz =
        "tod_contract2_ref<R,N,M,K,DimData>";
...
void tod_contract2_p1<R,N,M,K,DimData>::do_calculate() {
    ...
    dimensions<N+K> dimb(d.dimB());
    dimensions<M+K> dimc(d.dimC());
    dense_tensor<N+M, double, allocator<double> > ta(dima);
    dense_tensor<N+K, double, allocator<double> > tb(dimb);
    dense_tensor<M+K, double, allocator<double> > tc(dimc);
...
#endif // LIBTENSOR_TOD_CONTRACT2_PERFORMANCE_H

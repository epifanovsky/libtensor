...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2> class eval_contract;
...
public:
	typedef eval_contract<N, M, K, T, Expr1, Expr2>
...
public:
	core_contract(const letter_expr<K> &contr, const Expr1 &expr1,
		const Expr2 &expr2);
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
const char *core_contract<N, M, K, T, Expr1, Expr2>::k_clazz =
	"core_contract<N, M, K, T, Expr1, Expr2>";
...
public:
	...
	typedef core_contract<N, M, K, T, Expr1, Expr2> core_t;
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
template<int Dummy>
struct eval_contract<N, M, K, T, Expr1, Expr2>::narg<oper_tag, Dummy> {
	static const size_t k_narg = 1;
};

template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
core_contract<N, M, K, T, Expr1, Expr2>::core_contract(
	const letter_expr<K> &contr, const Expr1 &expr1, const Expr2 &expr2) :
		m_contr(contr),
		m_expr1(expr1),
		m_expr2(expr2) {

	static const char *method = "core_contract("
		"const letter_expr<K>&, const Expr1&, const Expr2&)";
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
inline bool core_contract<N, M, K, T, Expr1, Expr2>::contains(
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
inline size_t core_contract<N, M, K, T, Expr1, Expr2>::index_of(
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
inline const letter&core_contract<N, M, K, T, Expr1, Expr2>::letter_at(
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
eval_contract<N, M, K, T, Expr1, Expr2>::eval_contract(expression_t &expr,
	labeled_btensor<k_orderc, T, true> &result) throw(exception) :
		m_expr(expr),
		m_contr(mk_contr(expr, result))//,
		/*m_op(m_contr)*/ {
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
template<typename Tag>
arg<N + M, T, Tag> eval_contract<N, M, K, T, Expr1, Expr2>::get_arg(
	const Tag &tag, size_t i) const throw(exception) {

	throw_exc("eval_contract<N, M, K, T, Expr1, Expr2>",
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
arg<N + M, T, oper_tag> eval_contract<N, M, K, T, Expr1, Expr2>::get_arg(
...
template<size_t N, size_t M, size_t K, typename T, typename Expr1,
	typename Expr2>
contraction2<N, M, K>
eval_contract<N, M, K, T, Expr1, Expr2>::mk_contr(
...
/**	\brief Contraction of two tensors over one index
	...
	\ingroup libtensor_btensor_expr
 **/
template<size_t N, size_t M, typename T, bool Assignable1, bool Assignable2>
expr<N + M - 2, T, core_contract<N - 1, M - 1, 1, T,
	expr< N, T, core_ident<N, T, Assignable1> >,
	expr< M, T, core_ident<M, T, Assignable2> >
> >
inline contract(
	const letter &let,
	labeled_btensor<N, T,Assignable1> bta,
	labeled_btensor<M, T, Assignable2> btb) {

	typedef letter_expr<1> label_t;
	typedef core_ident<N, T, Assignable1> id1_t;
	typedef expr<N, T, id1_t> expr1_t;
	typedef core_ident<M, T, Assignable2> id2_t;
	typedef expr<M, T, id2_t> expr2_t;
	typedef core_contract<N - 1, M - 1, 1, T, expr1_t, expr2_t> contract_t;
...
/**	\brief Contraction of two tensors over multiple indexes
	...
	\ingroup libtensor_btensor_expr
 **/
template<size_t K, size_t N, size_t M, typename T, bool Assignable1,
	bool Assignable2>
...
inline contract(
	...
	typedef expr< N, T, core_ident<N, T, Assignable1> > expr1_t;
	typedef expr< M, T, core_ident<M, T, Assignable2> > expr2_t;
	typedef core_contract<N - K, M - K, K, T, expr1_t, expr2_t>
		contract_t;
	typedef expr<N + M - 2 * K, T, contract_t> expr_t;
	return expr_t(contract_t(contr, expr1_t(bta), expr2_t(btb)));
}

template<size_t K, size_t N, size_t M, typename T, typename Expr1,
	bool Assignable2>
expr<N + M - 2 * K, T, core_contract<N - K, M - K, K, T,
	expr<N, T, Expr1>,
	expr< M, T, core_ident<M, T, Assignable2> >
